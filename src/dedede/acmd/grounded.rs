...
unsafe extern "C" fn ssbexo_dedede_down_taunt_expression(agent: &mut L2CAgentBase) {
    ...
    if ArticleModule::is_exist(agent.module_accessor, *FIGHTER_DEDEDE_GENERATE_ARTICLE_MASK) {
        if macros::is_excute(agent) {
            ArticleModule::remove_exist(agent.module_accessor, *FIGHTER_DEDEDE_GENERATE_ARTICLE_WADDLEDEE, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
            ArticleModule::set_visibility_whole(agent.module_accessor, *FIGHTER_DEDEDE_GENERATE_ARTICLE_WADDLEDEE, false, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
            slope!(agent, *MA_MSC_CMD_SLOPE_SLOPE_INTP, *SLOPE_STATUS_R, 4);
        }
    }
    else {
        if macros::is_excute(agent) {
            ...
            ArticleModule::set_visibility_whole(agent.module_accessor, *FIGHTER_DEDEDE_GENERATE_ARTICLE_WADDLEDEE, true, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
...
unsafe extern "C" fn ssbexo_dedede_jab_1_acmd(agent: &mut L2CAgentBase) {
    MotionModule::set_rate(agent.module_accessor, 1.66);
    frame(agent.lua_state_agent, 10.0);
    if macros::is_excute(agent) {
