...
unsafe extern "C" fn popo_special_s_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    StatusModule::init_settings(fighter.module_accessor, smash::app::SituationKind(*SITUATION_KIND_NONE), *FIGHTER_KINETIC_TYPE_NONE, *GROUND_CORRECT_KIND_KEEP as u32, smash::app::GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_ALWAYS_BOTH_SIDES), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLOAT, 0);
...
unsafe extern "C" fn popo_special_s_partner_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn popo_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("popo")
    .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_S, popo_special_s_pre_status)
    .status(Pre, *FIGHTER_POPO_STATUS_KIND_SPECIAL_S_PARTNER, popo_special_s_partner_pre_status)
    .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_LW, popo_special_lw_pre_status)
    .install()
    ;
}