...
unsafe extern "C" fn marth_down_special_attack_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    if WorkModule::is_flag(fighter.module_accessor, FIGHTER_INSTANCE_WORK_ID_FLAG_AUTO_COUNTER) {
        ...
        fighter.sub_shift_status_main(L2CValue::Ptr(marth_down_special_attack_loop as *const () as _))
    }
    else {
...
unsafe extern "C" fn marth_down_special_attack_loop(fighter: &mut L2CFighterCommon) -> bool {
...
pub fn install() {
    Agent::new("marth")
    .status(Main, *FIGHTER_MARTH_STATUS_KIND_SPECIAL_LW_HIT, marth_down_special_attack_main_status)
    .install()
    ;
}