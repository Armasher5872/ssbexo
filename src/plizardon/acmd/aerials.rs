...
unsafe extern "C" fn ssbexo_plizardon_fair_sound(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::PLAY_SE(agent, Hash40::new("se_plizardon_attackair_s01"));
    }
}
...
pub fn install() {
    ...
    .sound_acmd("sound_attackairf", ssbexo_plizardon_fair_sound, Priority::Low)
    .expression_acmd("expression_attackairf", ssbexo_plizardon_fair_expression, Priority::Low)
    .install()
    ;
}