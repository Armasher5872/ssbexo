...
unsafe extern "C" fn mario_special_s_loop_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if prev_status_kind == *FIGHTER_STATUS_KIND_SPECIAL_S {
        sv_kinetic_energy!(set_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, 1.3*lr, 0.0);
    }
    ...
    sv_kinetic_energy!(set_limit_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, 1.3, 0.0);
    sv_kinetic_energy!(set_accel, fighter, FIGHTER_KINETIC_ENERGY_ID_GRAVITY, -gravity);
    sv_kinetic_energy!(set_limit_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_GRAVITY, 2.69);
    0.into()
}
...
unsafe extern "C" fn mario_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        ...
        && prev_situation_kind == *SITUATION_KIND_AIR {
            GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
            KineticModule::change_kinetic(fighter.module_accessor, *FIGHTER_KINETIC_TYPE_GROUND_STOP);
            WorkModule::set_float(fighter.module_accessor, 10.0, *FIGHTER_STATUS_LANDING_WORK_FLOAT_STIFFNESS_FRAME);
            fighter.change_status(FIGHTER_STATUS_KIND_LANDING.into(), false.into());
        }
        if situation_kind == *SITUATION_KIND_AIR
        && prev_situation_kind == *SITUATION_KIND_GROUND {
