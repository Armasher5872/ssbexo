...
unsafe extern "C" fn roy_start_initialization(vtable: u64, fighter: &mut Fighter) {
    ...
    if fighter.battle_object.kind == *FIGHTER_KIND_ROY as u32 {
        ...
        lua_module_fighter.global_table[STATUS_END_CONTROL].assign(&L2CValue::Ptr(roy_end_control as *const () as _));
    }
...
unsafe extern "C" fn roy_opff(vtable: u64, fighter: &mut Fighter) -> u64 {
    if fighter.battle_object.kind == *FIGHTER_KIND_ROY as u32 {
        ...
        if WorkModule::is_flag(boma, FIGHTER_ROY_INSTANCE_WORK_ID_FLAG_SOL_ACTIVE) {
            ...
            if sol_timer > 0 {
                ...
                if [300, 270, 240, 210, 180, 150, 120, 90, 60, 30].contains(&sol_timer) {
                    ...
                    macros::EFFECT_FOLLOW_NO_STOP(opponent_agent, Hash40::new("roy_fire"), Hash40::new("top"), 0, 7, 0, 0, 0, 0, 1, true);
