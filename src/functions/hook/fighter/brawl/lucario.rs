...
const LUCARIO_VTABLE_START_INITIALIZATION_OFFSET: usize = 0xc5b730;
const LUCARIO_VTABLE_RESET_INITIALIZATION_OFFSET: usize = 0xc5b8f0;
const LUCARIO_VTABLE_ONCE_PER_FIGHTER_FRAME_OFFSET: usize = 0xc5ce40;
...
unsafe extern "C" fn lucario_opff(vtable: u64, fighter: &mut Fighter) {
    ...
    if [hash40("appeal_hi_r"), hash40("appeal_hi_l")].contains(&motion_kind) && frame > 16.0 && !MEGA_EVOLVE[entry_id] {
        MEGA_EVOLVE[entry_id] = true;
    }
    if [hash40("appeal_lw_r"), hash40("appeal_lw_l")].contains(&motion_kind) && frame > 43.0 && MEGA_EVOLVE[entry_id] != false {
        MEGA_EVOLVE[entry_id] = false;
    }
    if status_kind == *FIGHTER_STATUS_KIND_FINAL {
        MEGA_EVOLVE[entry_id] = false;
    }
    if MEGA_EVOLVE[entry_id] {
