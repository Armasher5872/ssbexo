...
unsafe extern "C" fn ken_opff(vtable: u64, fighter: &mut Fighter) {
    ...
    let boma = fighter.battle_object.module_accessor;
    let agent = get_fighter_common_from_accessor(&mut *boma);
    if agent.global_table[FIGHTER_KIND].get_i32() == *FIGHTER_KIND_KEN {
        ...
        println!("Command Input Timer: {}", command_input_timer);
        println!("Attack Command 1 Counter: {}", attack_command1_counter);
    }
}
