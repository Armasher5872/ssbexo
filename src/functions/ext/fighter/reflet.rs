...
pub unsafe extern "C" fn is_goeita(object_boma: *mut BattleObjectModuleAccessor) -> bool {
    if utility::get_kind(&mut *object_boma) == *WEAPON_KIND_KOOPAJR_CANNONBALL {
        ...
        if owner_kind == *FIGHTER_KIND_REFLET {
            return true;
        }
    }
    return false;
...
pub unsafe extern "C" fn should_remove_goeita_on_hit(weapon: &mut L2CWeaponCommon) -> bool {
    if AttackModule::is_infliction_status(weapon.module_accessor, WEAPON_REFLET_GOEITA_STATUS_KIND_SHOOT)
...
pub unsafe extern "C" fn goeita_removal(weapon: &mut L2CWeaponCommon) {
...
pub unsafe extern "C" fn goeita_hit_removal(weapon: &mut L2CWeaponCommon) {
    let pos = *PostureModule::pos(weapon.module_accessor);
