...
unsafe extern "C" fn rockman_rockbuster_shoot_walk_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(fighter.module_accessor, SituationKind(*SITUATION_KIND_GROUND), *FIGHTER_KINETIC_TYPE_MOTION, *GROUND_CORRECT_KIND_GROUND_OTTOTTO as u32, GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, keep_flag, keep_int, keep_float, *FS_SUCCEEDS_KEEP_VISIBILITY);
    FighterStatusModuleImpl::set_fighter_status_data(fighter.module_accessor, false, *FIGHTER_TREADED_KIND_ENABLE, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK | *FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_ATTACK_S3 | *FIGHTER_LOG_MASK_FLAG_SHOOT) as u64, (*FIGHTER_STATUS_ATTR_SCALE_KINETIC_ENERGY | *FIGHTER_STATUS_ATTR_INTO_DOOR | *FIGHTER_STATUS_ATTR_DISABLE_JUMP_BOARD_EFFECT) as u32, *FIGHTER_POWER_UP_ATTACK_BIT_ATTACK_3 as u32, 0);
    0.into()
}

unsafe extern "C" fn rockman_rockbuster_shoot_walk_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let prev_status = fighter.global_table[PREV_STATUS_KIND].get_i32();
    ...
    if prev_status != *FIGHTER_STATUS_KIND_WALK {
        ...
        fighter.init_walk_speed(walk_speed_max.into(), FIGHTER_STATUS_WALK_WORK_FLOAT_SPEED.into(), 1.0f32.into(), false.into(), hash40("attack_s3_fast").into(), hash40("attack_s3_middle").into(), hash40("attack_s3_slow").into());
        fighter.init_walk_motion(hash40("attack_s3_fast").into(), hash40("attack_s3_middle").into(), hash40("attack_s3_slow").into(), FIGHTER_STATUS_WALK_WORK_FLOAT_SPEED.into(), 1.0f32.into());
    }
...
unsafe extern "C" fn rockman_rockbuster_shoot_walk_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.calc_walk_speed(walk_speed.into(), walk_speed_max.into(), walk_accel_mul.into(), walk_accel_add.into(), 0.0f32.into(), walk_accel_mul_common.into(), FIGHTER_STATUS_WALK_WORK_FLOAT_SPEED.into(), 1.0f32.into(), true.into());
    fighter.set_walk_motion(hash40("attack_s3_fast").into(), hash40("attack_s3_middle").into(), hash40("attack_s3_slow").into(), FIGHTER_STATUS_WALK_WORK_FLOAT_SPEED.into(), 1.0f32.into(), L2CValue::Ptr(rockman_rockbuster_walk_mot_helper as *const () as _));
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        if helper_ret && WorkModule::is_enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_WALK) && sit == *SITUATION_KIND_GROUND {
        ...
        if fighter.sub_check_button_jump().get_bool() && ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) && !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_ROCKMAN_INSTANCE_WORK_ID_FLAG_ROCKBUSTER_PREV_ESCAPE) {
...
unsafe extern "C" fn rockman_rockbuster_shoot_walk_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("rockman")
    .status(Pre, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WALK, rockman_rockbuster_shoot_walk_pre_status)
    .status(Main, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WALK, rockman_rockbuster_shoot_walk_main_status)
    .status(End, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WALK, rockman_rockbuster_shoot_walk_end_status)
    .install()
    ;
}