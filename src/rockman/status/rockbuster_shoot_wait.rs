...
unsafe extern "C" fn rockman_rockbuster_shoot_wait_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(fighter.module_accessor, false, *FIGHTER_TREADED_KIND_ENABLE, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK | *FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_ATTACK11 | *FIGHTER_LOG_MASK_FLAG_SHOOT) as u64, (*FIGHTER_STATUS_ATTR_SCALE_KINETIC_ENERGY | *FIGHTER_STATUS_ATTR_INTO_DOOR | *FIGHTER_STATUS_ATTR_DISABLE_JUMP_BOARD_EFFECT | 0) as u32, *FIGHTER_POWER_UP_ATTACK_BIT_ATTACK_1 as u32, status_data_arg);
    0.into()
}

unsafe extern "C" fn rockman_rockbuster_shoot_wait_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let prev_status_kind = fighter.global_table[PREV_STATUS_KIND].get_i32();
    ControlModule::reset_trigger(fighter.module_accessor);
    WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_WAIT);
    WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_SQUAT);
    WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_SQUAT_BUTTON);
    let was_walk = prev_status_kind == *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WALK;
...
unsafe extern "C" fn rockman_rockbuster_shoot_wait_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        if helper_ret && WorkModule::is_enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_WAIT) && sit == *SITUATION_KIND_GROUND {
        ...
        if fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_TURN != 0 && !helper_ret && rockman_rockbuster_can_turn_helper(fighter).get_bool() {
        ...
        if walk_stick_x < stick_x * lr {
            let status = if helper_ret {FIGHTER_STATUS_KIND_WALK} else {FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WALK};
            fighter.change_status(status.into(), true.into());
            return 1.into();
        }
    }
...
unsafe extern "C" fn rockman_rockbuster_shoot_wait_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("rockman")
    .status(Pre, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WAIT, rockman_rockbuster_shoot_wait_pre_status)
    .status(Main, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WAIT, rockman_rockbuster_shoot_wait_main_status)
    .status(End, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WAIT, rockman_rockbuster_shoot_wait_end_status)
    .install()
    ;
}