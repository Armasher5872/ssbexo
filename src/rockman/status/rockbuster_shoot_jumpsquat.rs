...
unsafe extern "C" fn rockman_rockbuster_shoot_jump_squat_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn rockman_rockbuster_shoot_jump_squat_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    MotionModule::change_motion(fighter.module_accessor, Hash40::new("jump_squat"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn rockman_rockbuster_shoot_jump_squat_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if MotionModule::is_end(fighter.module_accessor) {
        let status = if helper_ret {FIGHTER_STATUS_KIND_JUMP} else {FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_JUMP};
...
unsafe extern "C" fn rockman_rockbuster_shoot_jump_squat_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("rockman")
    .status(Pre, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_JUMP_SQUAT, rockman_rockbuster_shoot_jump_squat_pre_status)
    .status(Main, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_JUMP_SQUAT, rockman_rockbuster_shoot_jump_squat_main_status)
    .status(End, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_JUMP_SQUAT, rockman_rockbuster_shoot_jump_squat_end_status)
    .install()
    ;
}