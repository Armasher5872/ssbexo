...
#[weapon_frame( agent = WEAPON_KIND_PICHU_DENGEKI )]
fn pichu_dengeki_functions(fighter: &mut L2CFighterBase) {
	unsafe {
		let module_accessor = smash::app::sv_system::battle_object_module_accessor(fighter.lua_state_agent);
...
#[weapon_frame( agent = WEAPON_KIND_PICHU_DENGEKIDAMA )]
fn pichu_dengekidama_functions(fighter: &mut L2CFighterBase) {
	unsafe {
		let module_accessor = smash::app::sv_system::battle_object_module_accessor(fighter.lua_state_agent);
		let owner_module_accessor = &mut *sv_battle_object::module_accessor((WorkModule::get_int(module_accessor, *WEAPON_INSTANCE_WORK_ID_INT_LINK_OWNER)) as u32);
		let weapon_kind = smash::app::utility::get_kind(module_accessor) as i32;
...
pub fn install() {
    install_agent_frames!{
        pichu_frame,
        pichu_dengeki_functions,
		pichu_dengekidama_functions
    };
}