...
unsafe fn ssbuexo_bayonetta_grab_acmd(fighter: &mut L2CAgentBase) {
...
unsafe fn ssbuexo_bayonetta_dash_grab_acmd(fighter: &mut L2CAgentBase) {
    ...
    if macros::is_excute(fighter) {
        ...
        macros::CATCH(fighter, 0, Hash40::new("top"), 2.6, 0.0, 6.6, 4.0, Some(0.0), Some(6.6), Some(10.9), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    macros::game_CaptureCutCommon(fighter);
    wait(fighter.lua_state_agent, 2.0);
    if macros::is_excute(fighter) {
...
unsafe fn ssbuexo_bayonetta_pivot_grab_acmd(fighter: &mut L2CAgentBase) {
    frame(fighter.lua_state_agent, 10.0);
    if macros::is_excute(fighter) {
        GrabModule::set_rebound(fighter.module_accessor, true);
    }
    ...
    if macros::is_excute(fighter) {
        ...
        macros::CATCH(fighter, 0, Hash40::new("top"), 3.3, 0.0, 6.6, -4.0, Some(0.0), Some(6.6), Some(-15.5), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    macros::game_CaptureCutCommon(fighter);
    wait(fighter.lua_state_agent, 2.0);
    if macros::is_excute(fighter) {
...
pub fn install() {
    install_acmd_scripts!(
        ssbuexo_bayonetta_grab_acmd,
        ssbuexo_bayonetta_dash_grab_acmd,
        ssbuexo_bayonetta_pivot_grab_acmd
    );
}