...
unsafe fn ssbuexo_shizue_grab_acmd(fighter: &mut L2CAgentBase) 
{
...
unsafe fn ssbuexo_shizue_dash_grab_acmd(fighter: &mut L2CAgentBase) 
{
...
unsafe fn ssbuexo_shizue_pivot_grab_acmd(fighter: &mut L2CAgentBase) 
{
    frame(fighter.lua_state_agent, 9.0);
    if macros::is_excute(fighter) {
        GrabModule::set_rebound(fighter.module_accessor, true);
...
pub fn install() {
    install_acmd_scripts!(
        ssbuexo_shizue_grab_acmd,
        ssbuexo_shizue_dash_grab_acmd,
        ssbuexo_shizue_pivot_grab_acmd
    );
}