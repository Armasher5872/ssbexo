...
unsafe extern "C" fn ssbexo_reflet_forward_smash_acmd(agent: &mut L2CAgentBase) {
    ...
    if WorkModule::is_flag(agent.module_accessor, *FIGHTER_REFLET_INSTANCE_WORK_ID_FLAG_THUNDER_SWORD_ON) {
        if macros::is_excute(agent) {
            macros::ATTACK(agent, 0, 0, Hash40::new("sword"), 10.0, 361, 80, 0, 60, 1.5, 0.0, 8.0, 0.0, Some(0.0), Some(2.0), Some(0.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_ELEC, *ATTACK_REGION_SWORD);
        }
    }
    else {
        if macros::is_excute(agent) {
...
pub fn install() {
    Agent::new("reflet")
    .game_acmd("game_attacks4", ssbexo_reflet_forward_smash_acmd, Priority::Low)
    .game_acmd("game_attackhi4", ssbexo_reflet_up_smash_acmd, Priority::Low)
    .game_acmd("game_attacklw4", ssbexo_reflet_down_smash_acmd, Priority::Low)
    .install()
    ;
}