...
unsafe extern "C" fn ssbexo_samus_down_throw_acmd(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
        macros::ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 8.0, 65, 75, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_elec_whip"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    ...
    frame(agent.lua_state_agent, 20.0);
    if macros::is_excute(agent) {
        macros::CHECK_FINISH_CAMERA(agent, 17.0, 0.0);
    }
    frame(agent.lua_state_agent, 21.0);
    if macros::is_excute(agent) {
