...
unsafe extern "C" fn mewtwo_frame(fighter: &mut L2CFighterCommon) {
    ...
    let cancel_statuses = [
        ...
        *FIGHTER_STATUS_KIND_ATTACK_LW4, *FIGHTER_STATUS_KIND_ATTACK_AIR, *FIGHTER_STATUS_KIND_SPECIAL_N, *FIGHTER_STATUS_KIND_SPECIAL_S, *FIGHTER_STATUS_KIND_SPECIAL_LW
    ].contains(&status_kind);
    if fighter.global_table[IS_STOP].get_bool() {
        if cancel_statuses && fighter.is_cat_flag(Cat1::SpecialHi) {
