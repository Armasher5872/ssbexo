...
unsafe extern "C" fn ssbexo_rosetta_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::CATCH(agent, 0, Hash40::new("top"), 3.8, 0.0, 8.0, 4.0, Some(0.0), Some(8.0), Some(10.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe extern "C" fn ssbexo_rosetta_pivot_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::CATCH(agent, 0, Hash40::new("top"), 3.8, 0.0, 8.0, -4.0, Some(0.0), Some(8.0), Some(-17.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    macros::game_CaptureCutCommon(agent);
    wait(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
