...
unsafe extern "C" fn ssbexo_pacman_up_tilt_effect(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 7.0);
    if macros::is_excute(agent) {
        macros::EFFECT(agent, Hash40::new("sys_attack_arc"), Hash40::new("top"), -2.0, 8.0, 4.5, 0, -95, -82, 1.0, 0, 0, 0, 0, 0, 0, true);
    }
}
...
unsafe extern "C" fn ssbexo_pacman_down_tilt_acmd(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 7.0);
    if macros::is_excute(agent) {
        macros::ATTACK(agent, 0, 0, Hash40::new("top"), 6.0, 45, 50, 0, 65, 6.0, 0.0, 3.6, 0.0, None, None, None, 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_F, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_PUNCH, *ATTACK_REGION_BODY);
...
pub fn install() {
    ...
    .sound_acmd("sound_attackhi3", ssbexo_pacman_up_tilt_sound, Priority::Low)
    .expression_acmd("expression_attackhi3", ssbexo_pacman_up_tilt_expression, Priority::Low)
    .game_acmd("game_attacklw3", ssbexo_pacman_down_tilt_acmd, Priority::Low)
    .install()
    ;
}