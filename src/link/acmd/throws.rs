...
unsafe fn ssbuexo_link_grab_acmd(fighter: &mut L2CAgentBase) {
...
unsafe fn ssbuexo_link_dash_grab_acmd(fighter: &mut L2CAgentBase) {
    ...
    if macros::is_excute(fighter) {
        ...
        macros::CATCH(fighter, 0, Hash40::new("top"), 2.6, 0.0, 7.0, 4.0, Some(0.0), Some(7.0), Some(9.4), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    macros::game_CaptureCutCommon(fighter);
    wait(fighter.lua_state_agent, 2.0);
    if macros::is_excute(fighter) {
...
unsafe fn ssbuexo_link_pivot_grab_acmd(fighter: &mut L2CAgentBase) {
    frame(fighter.lua_state_agent, 9.0);
    if macros::is_excute(fighter) {
        GrabModule::set_rebound(fighter.module_accessor, true);
    }
...
pub fn install() {
    install_acmd_scripts!(
        ssbuexo_link_grab_acmd,
        ssbuexo_link_dash_grab_acmd,
        ssbuexo_link_pivot_grab_acmd
    );
}