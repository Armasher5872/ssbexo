...
unsafe fn ssbuexo_jack_grab_acmd(fighter: &mut L2CAgentBase) {
    ...
    if macros::is_excute(fighter) {
        ...
        macros::CATCH(fighter, 0, Hash40::new("top"), 3.3, 0.0, 8.1, 4.5, Some(0.0), Some(8.1), Some(9.0), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe fn ssbuexo_jack_dash_grab_acmd(fighter: &mut L2CAgentBase) {
...
unsafe fn ssbuexo_jack_pivot_grab_acmd(fighter: &mut L2CAgentBase) {
    frame(fighter.lua_state_agent, 8.0);
    if macros::is_excute(fighter) {
        GrabModule::set_rebound(fighter.module_accessor, true);
    }
    ...
    if macros::is_excute(fighter) {
        ...
        macros::CATCH(fighter, 0, Hash40::new("top"), 3.3, 0.0, 6.6, -7.0, Some(0.0), Some(6.6), Some(-16.7), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    macros::game_CaptureCutCommon(fighter);
    wait(fighter.lua_state_agent, 2.0);
    if macros::is_excute(fighter) {
...
pub fn install() {
    install_acmd_scripts!(
        ssbuexo_jack_grab_acmd,
        ssbuexo_jack_dash_grab_acmd,
        ssbuexo_jack_pivot_grab_acmd
    );
}