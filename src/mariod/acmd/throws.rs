...
//Standing Grab ACMD
unsafe extern "C" fn ssbexo_mariod_grab_acmd(agent: &mut L2CAgentBase) {
...
//Dash Grab ACMD
unsafe extern "C" fn ssbexo_mariod_dash_grab_acmd(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
        GrabModule::set_rebound(agent.module_accessor, true);
    }
...
//Pivot Grab ACMD
unsafe extern "C" fn ssbexo_mariod_pivot_grab_acmd(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
        GrabModule::set_rebound(agent.module_accessor, true);
    }
...
pub fn install() {
    Agent::new("mariod")
    .game_acmd("game_catch", ssbexo_mariod_grab_acmd, Priority::Low)
    .game_acmd("game_catchdash", ssbexo_mariod_dash_grab_acmd, Priority::Low)
    .game_acmd("game_catchturn", ssbexo_mariod_pivot_grab_acmd, Priority::Low)
    .install()
    ;
}