...
unsafe extern "C" fn jack_dispatch_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("dispatch"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn jack_dispatch_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    VisibilityModule::set_material_anim_priority(boma, Hash40::new("mask"), false);
    VisibilityModule::set_status_default_int64(boma, hash40("mask") as i64, hash40("on") as i64);
    WorkModule::off_flag(boma, *FIGHTER_JACK_INSTANCE_WORK_ID_FLAG_DOYLE);
    0.into()
}
