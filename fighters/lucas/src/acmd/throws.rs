...
unsafe extern "C" fn ssbexo_lucas_forward_throw_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 10.0, 36, 59, 0, 75, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
...
unsafe extern "C" fn ssbexo_lucas_down_throw_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        FT_MOTION_RATE(agent, 0.927);
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 12.0, 70, 30, 0, 95, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_CATCH, 0, 3.0, 361, 100, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
...
pub fn install() {
    ...
    .game_acmd("game_catch", ssbexo_lucas_grab_acmd, Low)
    .game_acmd("game_catchdash", ssbexo_lucas_dash_grab_acmd, Low)
    .game_acmd("game_catchturn", ssbexo_lucas_pivot_grab_acmd, Low)
    .game_acmd("game_throwf", ssbexo_lucas_forward_throw_acmd, Low)
    .game_acmd("game_throwlw", ssbexo_lucas_down_throw_acmd, Low)
    .install()
    ;
}