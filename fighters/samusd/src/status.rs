...
unsafe extern "C" fn samusd_special_hi_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    StatusModule::init_settings(boma, smash::app::SituationKind(*SITUATION_KIND_AIR), *FIGHTER_KINETIC_TYPE_MOTION_FALL, *GROUND_CORRECT_KIND_AIR as u32, smash::app::GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLOAT, 0);
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_SPECIAL_HI | *FIGHTER_LOG_MASK_FLAG_ACTION_TRIGGER_ON) as u64, (*FIGHTER_STATUS_ATTR_START_TURN | *FIGHTER_STATUS_ATTR_INTO_DOOR) as u32, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_HI as u32, 0);
    0.into()
}
...
unsafe extern "C" fn samusd_special_hi_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let ray_check_hit_pos = GroundModule::ray_check_hit_pos(boma, &Vector2f{x: pos_x, y: pos_y}, &Vector2f{x: 0.0, y: -6.0}, result, true);
    ...
    if ray_check_hit_pos {
        ...
        PostureModule::set_pos(boma, &Vector3f{x: pos_x, y: ray_check_y + uniq_float_start_y, z: pos_z});
    }
...
unsafe extern "C" fn samusd_special_hi_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let float_time = WorkModule::get_int(boma, *FIGHTER_SAMUSD_INSTANCE_WORK_ID_INT_FLOAT_TIME);
    ...
    if frame > 22.0 {
        ...
        && cmd_cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_HI != 0 || cmd_cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_LW != 0 || cmd_cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_S_L != 0 || cmd_cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_S_R != 0 {
            MotionModule::change_motion(boma, Hash40::new("special_air_hi_appeal"), 0.0, 1.0, false, 0.0, false, false);
        }
    }
    if float_time < 50 {
        WorkModule::inc_int(boma, *FIGHTER_SAMUSD_INSTANCE_WORK_ID_INT_FLOAT_TIME);
    }
    if float_time >= 50 {
...
unsafe extern "C" fn samusd_special_hi_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    STOP_SE(fighter, Hash40::new("se_samusd_appear01"));
    0.into()
}
