...
unsafe extern "C" fn ssbexo_samusd_forward_throw_acmd(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 5.0, 15, 40, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new_raw(0x13313725f6), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_CATCH, 0, 3.0, 361, 100, 0, 50, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new_raw(0x13313725f6), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
    ...
    if is_excute(agent) {
        ...
        ATTACK(agent, 0, 0, Hash40::new("armr"), 0.1, 0, 0, 0, 0, 9.0, 0.0, 0.0, 0.0, None, None, None, 0.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, true, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal_poison"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_FIRE, *ATTACK_REGION_BODY);
...
unsafe extern "C" fn ssbexo_samusd_forward_throw_effect(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 12.0);
    if is_excute(agent) {
        EFFECT_FLW_POS(agent, Hash40::new("samusd_throw_hi"), Hash40::new("armr"), 8, 0, 0, 0, 0, 0, 0.6, true);
...
unsafe extern "C" fn ssbexo_samusd_back_throw_acmd(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 7.0, 45, 40, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_B, 0.0, true, Hash40::new("collision_attr_elec_whip"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_CATCH, 0, 3.0, 361, 100, 0, 50, 0.0, 1.0, *ATTACK_LR_CHECK_B, 0.0, true, Hash40::new("collision_attr_elec_whip"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    }
    frame(lua_state, 23.0);
    for _ in 0..12 {
        if is_excute(agent) {
    ...
    if is_excute(agent) {
        CHECK_FINISH_CAMERA(agent, 46, 28);
...
unsafe extern "C" fn ssbexo_samusd_back_throw_effect(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        LANDING_EFFECT_FLIP(agent, Hash40::new("sys_sliding_smoke"), Hash40::new("sys_sliding_smoke"), Hash40::new("top"), 0, 0, 0, 0, 0, 0, 1.3, 0, 0, 0, 0, 0, 0, false, *EF_FLIP_NONE);
        LAST_EFFECT_SET_RATE(agent, 0.7);
    }
    frame(lua_state, 47.0);
    if is_excute(agent) {
...
unsafe extern "C" fn ssbexo_samusd_back_throw_sound(agent: &mut L2CAgentBase) {
    ...
    for _ in 0..12 {
        if is_excute(agent) {
            PLAY_SE(agent, Hash40::new("se_common_chip"));
        }
        wait(lua_state, 2.0);
    }
    frame(lua_state, 48.0);
...
unsafe extern "C" fn ssbexo_samusd_up_throw_acmd(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 8.0, 75, 70, 0, 70, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_elec_whip"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    ...
    frame(lua_state, 31.0);
    if is_excute(agent) {
        CHECK_FINISH_CAMERA(agent, 9, 56);
        smash::app::lua_bind::FighterCutInManager::set_throw_finish_zoom_rate(singletons::FighterCutInManager(), 1.2);
...
unsafe extern "C" fn ssbexo_samusd_up_throw_effect(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        EFFECT_FOLLOW(agent, Hash40::new("samusd_gbeam_vanish"), Hash40::new("armr"), 7, 0, 0, 0, 0, 0, 0.6, true);
    }
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_samusd_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_samusd_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_samusd_pivot_grab_acmd, Low)
    .acmd("game_catchattack", ssbexo_samusd_pummel_acmd, Low)
    .acmd("game_throwf", ssbexo_samusd_forward_throw_acmd, Low)
    .acmd("effect_throwf", ssbexo_samusd_forward_throw_effect, Low)
    .acmd("sound_throwf", ssbexo_samusd_forward_throw_sound, Low)
    .acmd("game_throwb", ssbexo_samusd_back_throw_acmd, Low)
    .acmd("effect_throwb", ssbexo_samusd_back_throw_effect, Low)
    .acmd("sound_throwb", ssbexo_samusd_back_throw_sound, Low)
    .acmd("game_throwhi", ssbexo_samusd_up_throw_acmd, Low)
    .acmd("effect_throwhi", ssbexo_samusd_up_throw_effect, Low)
    .acmd("sound_throwhi", ssbexo_samusd_up_throw_sound, Low)
    .install()
    ;
}