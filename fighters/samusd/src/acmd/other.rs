...
//Airdodge ACMD
unsafe extern "C" fn ssbexo_samusd_airdodge_acmd(_agent: &mut L2CAgentBase) {}
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL_CHARGE) {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_DISABLE_FINAL_START_CAMERA) {
            if is_excute(agent) {
                ...
                if is_excute(agent) {
                    CAM_ZOOM_IN_arg5(agent, 1.7, 0.0, 5.0*scale, 0.0, 0.0);
...
unsafe extern "C" fn ssbexo_samusd_final_smash_acmd(agent: &mut L2CAgentBase) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL_CHARGE) {
        if !IS_EXIST_ARTICLE(agent, *FIGHTER_SAMUSD_GENERATE_ARTICLE_LASER2) {
            if is_excute(agent) {
                ArticleModule::shoot(boma, *FIGHTER_SAMUSD_GENERATE_ARTICLE_LASER, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL), false);
                ...
                WorkModule::set_float(boma, -14.0, *FIGHTER_SAMUS_STATUS_FINAL_WORK_FLOAT_OY);
                ArticleModule::generate_article(boma, *FIGHTER_SAMUSD_GENERATE_ARTICLE_LASER2, false, -1);
                ArticleModule::shoot(boma, *FIGHTER_SAMUSD_GENERATE_ARTICLE_LASER2, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL), false);
            }
        }
        if is_excute(agent) {
...
unsafe extern "C" fn ssbexo_samusd_laser_shoot_acmd(agent: &mut L2CAgentBase) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL_CHARGE) {
        ...
        if is_excute(agent) {
            ...
            AttackModule::set_final_finish_cut_in(boma, 0, true, true, -1.0, false);
        }
        wait(lua_state, 8.0);
        if is_excute(agent) {
            AttackModule::clear_all(boma);
...
pub fn install() {
    ...
    .acmd("game_escapeair", ssbexo_samusd_airdodge_acmd, Low)
    .acmd("game_escapeairslide", ssbexo_samusd_airdodge_acmd, Low)
    .acmd("game_finalstart", ssbexo_samusd_final_smash_start_acmd, Low)
    .acmd("game_finalairstart", ssbexo_samusd_final_smash_start_acmd, Low)
    .acmd("game_final", ssbexo_samusd_final_smash_acmd, Low)
    .acmd("game_finalair", ssbexo_samusd_final_smash_acmd, Low)
    .install()
    ;
    Agent::new("samusd_laser")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .acmd("game_shoot", ssbexo_samusd_laser_shoot_acmd, Low)
    .install()
    ;
    Agent::new("samusd_laser2")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .acmd("game_lower", ssbexo_samusd_laser_2_lower_acmd, Low)
    .acmd("game_upper", ssbexo_samusd_laser_2_upper_acmd, Low)
    .install()
    ;
}