...
unsafe extern "C" fn ssbexo_samusd_nair_acmd(agent: &mut L2CAgentBase) {
    ...
    for _ in 0..10 {
        if is_excute(agent) {
            ...
            AttackModule::set_poison_param(boma, 0, 300, 30, 0.5, false);  
        }
        wait(lua_state, 1.0);
        if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attackairn", ssbexo_samusd_nair_acmd, Low)
    .acmd("effect_attackairn", ssbexo_samusd_nair_effect, Low)
    .acmd("game_attackairf", ssbexo_samusd_fair_acmd, Low)
    .acmd("effect_attackairf", ssbexo_samusd_fair_effect, Low)
    .acmd("sound_attackairf", ssbexo_samusd_fair_sound, Low)
    .acmd("game_attackairb", ssbexo_samusd_bair_acmd, Low)
    .acmd("effect_attackairb", ssbexo_samusd_bair_effect, Low)
    .acmd("sound_attackairb", ssbexo_samusd_bair_sound, Low)
    .acmd("game_attackairhi", ssbexo_samusd_uair_acmd, Low)
    .acmd("effect_attackairhi", ssbexo_samusd_uair_effect, Low)
    .acmd("sound_attackairhi", ssbexo_samusd_uair_sound, Low)
    .acmd("game_attackairlw", ssbexo_samusd_dair_acmd, Low)
    .acmd("effect_attackairlw", ssbexo_samusd_dair_effect, Low)
    .install()
    ;
}