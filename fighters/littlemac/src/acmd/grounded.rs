...
unsafe extern "C" fn ssbexo_littlemac_jab_1_acmd(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 9.0);
    if is_excute(agent) {
        WorkModule::on_flag(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLAG_CAN_INPUT_DREAMLAND_EXPRESS);
    }
...
unsafe extern "C" fn ssbexo_littlemac_jab_3_acmd(agent: &mut L2CAgentBase) {
    ...
    FT_MOTION_RATE(agent, 1.0);
    if is_excute(agent) {
        let successful_dreamland_express_inputs = WorkModule::get_int(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SUCCESSFUL_DREAMLAND_EXPRESS_INPUTS);
        if successful_dreamland_express_inputs == 2 {
            WorkModule::on_flag(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLAG_DREAMLAND_EXPRESS);
            WorkModule::set_int(boma, 0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SUCCESSFUL_DREAMLAND_EXPRESS_INPUTS);
        }
    }
    frame(lua_state, 4.0);
    if WorkModule::is_flag(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLAG_DREAMLAND_EXPRESS) {
        if is_excute(agent) {
...
unsafe extern "C" fn ssbexo_littlemac_rapid_jab_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    loop {
        ssbexo_littlemac_rapid_jab_sub_acmd(agent);
        wait(lua_state, 2.0);
        ssbexo_littlemac_rapid_jab_sub_acmd(agent);
        wait(lua_state, 2.0);
        ssbexo_littlemac_rapid_jab_sub_acmd(agent);
...
//Rapid Jab Sub ACMD
unsafe extern "C" fn ssbexo_littlemac_rapid_jab_sub_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attack11", ssbexo_littlemac_jab_1_acmd, Low)
    .acmd("game_attack12", ssbexo_littlemac_jab_2_acmd, Low)
    .acmd("game_attack13", ssbexo_littlemac_jab_3_acmd, Low)
    .acmd("effect_attack13", ssbexo_littlemac_jab_3_effect, Low)
    .acmd("game_attack100", ssbexo_littlemac_rapid_jab_acmd, Low)
    .acmd("game_attackdash", ssbexo_littlemac_dash_attack_acmd, Low)
    .acmd("effect_attackdash", ssbexo_littlemac_dash_attack_effect, Low)
    .acmd("sound_attackdash", ssbexo_littlemac_dash_attack_sound, Low)
    .acmd("expression_attackdash", ssbexo_littlemac_dash_attack_expression, Low)
    .install()
    ;
}