...
unsafe extern "C" fn littlemac_status_attack100_main_uniq_func(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let step = WorkModule::get_int(boma, *FIGHTER_STATUS_ATTACK_WORK_INT_100_STEP);
    ...
    if 1 == jump_attack_frame {
        if !is_stop && WorkModule::get_int64(boma, *FIGHTER_STATUS_WORK_ID_INT_RESERVE_LOG_ATTACK_KIND) > 0 {
    ...
    if step == *FIGHTER_STATUS_ATTACK_100_STEP_START {
        ...
        if motion_kind == hash40("attack_100_start") && !MotionModule::is_end(boma) {
            return 0.into();
        }
        WorkModule::on_flag(boma, *FIGHTER_STATUS_ATTACK_FLAG_100_CONTINUE);
        WorkModule::off_flag(boma, *FIGHTER_STATUS_ATTACK_FLAG_100_CONTINUE_CHECK);
        WorkModule::off_flag(boma, *FIGHTER_STATUS_ATTACK_FLAG_100_CONTINUE_INPUT);
        MotionModule::change_motion(boma, Hash40::new("attack_100"), 0.0, 1.0, false, 0.0, false, false);
        WorkModule::set_int(boma, *FIGHTER_STATUS_ATTACK_100_STEP_LOOP, *FIGHTER_STATUS_ATTACK_WORK_INT_100_STEP);
    }
        ...
        if WorkModule::is_flag(boma, *FIGHTER_STATUS_ATTACK_FLAG_100_CONTINUE) {
            return 0.into();
        }
        ...
        WorkModule::set_int(boma, *FIGHTER_STATUS_ATTACK_100_STEP_END, *FIGHTER_STATUS_ATTACK_WORK_INT_100_STEP);
