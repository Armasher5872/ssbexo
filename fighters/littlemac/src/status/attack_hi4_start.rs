...
unsafe extern "C" fn littlemac_attack_hi4_start_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    littlemac_status_attack_hi4_start_common(fighter, hash40("attack_hi4").into())
}

unsafe extern "C" fn littlemac_status_attack_hi4_start_common(fighter: &mut L2CFighterCommon, motion_kind: L2CValue) -> L2CValue {
    fighter.sub_status_AttackHi4Start_common(motion_kind);
    fighter.sub_shift_status_main(L2CValue::Ptr(littlemac_attack_hi4_start_main_loop as *const () as _))
}

unsafe extern "C" fn littlemac_attack_hi4_start_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn littlemac_attack_hi4_start_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_ATTACK_HI4_START, littlemac_attack_hi4_start_main_status)
    .status(End, *FIGHTER_STATUS_KIND_ATTACK_HI4_START, littlemac_attack_hi4_start_end_status)
    .install()
    ;
}