...
unsafe extern "C" fn littlemac_attack_lw4_hold_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_AttackLw4Hold_common();
    fighter.sub_shift_status_main(L2CValue::Ptr(littlemac_attack_lw4_hold_main_loop as *const () as _))
}

unsafe extern "C" fn littlemac_attack_lw4_hold_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.status_AttackLw4Hold_main()
}

unsafe extern "C" fn littlemac_attack_lw4_hold_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_ATTACK_LW4_HOLD, littlemac_attack_lw4_hold_main_status)
    .status(End, *FIGHTER_STATUS_KIND_ATTACK_LW4_HOLD, littlemac_attack_lw4_hold_end_status)
    .install()
    ;
}