...
unsafe extern "C" fn littlemac_attack_s3_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_AttackS3Common();
    fighter.sub_shift_status_main(L2CValue::Ptr(littlemac_attack_s3_main_loop as *const () as _))
}

unsafe extern "C" fn littlemac_attack_s3_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn littlemac_attack_s3_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_ATTACK_S3, littlemac_attack_s3_main_status)
    .status(End, *FIGHTER_STATUS_KIND_ATTACK_S3, littlemac_attack_s3_end_status)
    .install()
    ;
}