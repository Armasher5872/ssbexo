...
unsafe extern "C" fn littlemac_special_s_jump_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_s_jump_x_speed_ = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_jump_x_speed_"));
    let special_s_jump_y_speed_ = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_jump_y_speed_"));
    let special_s_jump_x_speed_air_ = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_jump_x_speed_air_"));
    let special_s_jump_y_speed_air_ = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_jump_y_speed_air_"));
    let special_s_jump_brake_x_ = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_jump_brake_x_"));
    let special_s_end_landing_frame_ = WorkModule::get_param_int(boma, hash40("param_special_s"), hash40("special_s_end_landing_frame_"));
    let special_s_jump_landing_disable_frame_ = WorkModule::get_param_int(boma, hash40("param_special_s"), hash40("special_s_jump_landing_disable_frame_"));
    let air_accel_y = WorkModule::get_float(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_AIR_ACCEL_Y);
    let x_speed;
    let y_speed;
    let x = 0.0;
    let y = 0.0;
    ...
    WorkModule::off_flag(boma, *FIGHTER_LITTLEMAC_STATUS_SPECIAL_S_FLAG_IS_BLOW_SHIFT);
    if situation_kind != *SITUATION_KIND_GROUND {
        MotionModule::change_motion(boma, Hash40::new("special_air_s_jump"), 0.0, 1.0, false, 0.0, false, false);
        ...
        MotionModule::change_motion(boma, Hash40::new("special_s_jump"), 0.0, 1.0, false, 0.0, false, false);
    ...
    WorkModule::set_float(boma, special_s_end_landing_frame_ as f32, *FIGHTER_INSTANCE_WORK_ID_FLOAT_LANDING_FRAME);
    WorkModule::set_int(boma, special_s_jump_landing_disable_frame_, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SPECIAL_S_DISABLE_LANDING_FRAME);
    ...
    WorkModule::on_flag(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLAG_DISABLE_SPECIAL_S);
    vector["x"].assign(&L2CValue::F32(pos.x));
    vector["y"].assign(&L2CValue::F32(pos.y));
    vector["z"].assign(&L2CValue::F32(pos.z));
    WorkModule::set_float(boma, vec_y, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_SPECIAL_S_START_Y);
    WorkModule::set_int(boma, 0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SPECIAL_S_FRAME_COUNT);
...
unsafe extern "C" fn littlemac_special_s_jump_sub_status(fighter: &mut L2CFighterCommon, bool_check: L2CValue) -> L2CValue {
    ...
    if bool_check.get_bool() {
        WorkModule::dec_int(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SPECIAL_S_DISABLE_LANDING_FRAME);
        WorkModule::inc_int(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SPECIAL_S_FRAME_COUNT);
    }
    0.into()
}
...
unsafe extern "C" fn littlemac_special_s_jump_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let disable_landing_frame = WorkModule::get_int(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_INT_SPECIAL_S_DISABLE_LANDING_FRAME);
    let special_s_jump_attack_shift_frame_min = WorkModule::get_param_int(boma, hash40("param_special_s"), hash40("special_s_jump_attack_shift_frame_min"));
    ...
    if frame > special_s_jump_attack_shift_frame_min as f32 && (ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_SPECIAL) || ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_ATTACK)) {
        WorkModule::on_flag(boma, *FIGHTER_LITTLEMAC_STATUS_SPECIAL_S_FLAG_IS_BLOW_SHIFT);
    }
    ...
    if MotionModule::is_end(boma) || WorkModule::is_flag(boma, *FIGHTER_LITTLEMAC_STATUS_SPECIAL_S_FLAG_IS_BLOW_SHIFT) {
...
unsafe extern "C" fn littlemac_special_s_jump_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if status_kind != *FIGHTER_LITTLEMAC_STATUS_KIND_SPECIAL_S_BLOW {
        match star_punch_strength {
            1 => WorkModule::sub_float(boma, 1.0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_KO_GAGE),
            2 => WorkModule::sub_float(boma, 34.0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_KO_GAGE),
            3 => WorkModule::sub_float(boma, 100.0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_KO_GAGE),
            _ => WorkModule::sub_float(boma, 0.0, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLOAT_KO_GAGE)
        }
