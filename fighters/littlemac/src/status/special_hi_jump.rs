...
unsafe extern "C" fn littlemac_special_hi_jump_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_hi_dir_stick_x = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_dir_stick_x"));
    let special_hi_dir_mul = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_dir_mul"));
    ...
    let special_hi_landing_frame = WorkModule::get_param_int(boma, hash40("param_special_hi"), hash40("special_hi_landing_frame"));
    WorkModule::set_int64(boma, hash40("special_hi") as i64, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_INT_MOTION_KIND);
    WorkModule::set_int64(boma, hash40("special_hi") as i64, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_INT_MOTION_KIND_AIR);
    WorkModule::set_float(boma, special_hi_lr_stick_x, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_FLOAT_CONST_LR_STICK_X);
    WorkModule::set_float(boma, special_hi_dir_stick_x, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_FLOAT_CONST_DIR_STICK_X);
    WorkModule::set_float(boma, special_hi_dir_mul, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_FLOAT_CONST_DIR_MUL);
    ...
    WorkModule::set_int(boma, special_hi_landing_frame, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_INT_CONST_LANDING_FRAME);
    ...
    WorkModule::on_flag(boma, *FIGHTER_LITTLEMAC_STATUS_SPECIAL_HI_FLAG_MOVE_TRANS_TRIGGER);
...
unsafe extern "C" fn littlemac_special_hi_jump_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if frame == 6.0 && star_punch_strength > 0 {
        if WorkModule::is_flag(boma, *FIGHTER_LITTLEMAC_INSTANCE_WORK_ID_FLAG_SPECIAL_HI_FLAG_IS_START_AIR) {
            sv_kinetic_energy!(set_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_MOTION, get_motion_speed_x, get_motion_speed_y*1.6);
            sv_kinetic_energy!(set_limit_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_MOTION, 1.0, 5.0);
