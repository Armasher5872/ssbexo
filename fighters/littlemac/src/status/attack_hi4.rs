...
unsafe extern "C" fn littlemac_attack_hi4_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_AttackHi4_common(hash40("attack_hi4").into());
    fighter.sub_shift_status_main(L2CValue::Ptr(littlemac_attack_hi4_main_loop as *const () as _))
}

unsafe extern "C" fn littlemac_attack_hi4_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn littlemac_attack_hi4_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_ATTACK_HI4, littlemac_attack_hi4_main_status)
    .status(End, *FIGHTER_STATUS_KIND_ATTACK_HI4, littlemac_attack_hi4_end_status)
    .install()
    ;
}