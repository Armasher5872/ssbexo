...
unsafe extern "C" fn ssbexo_diddy_win_3_acmd(agent: &mut L2CAgentBase) {
    ...
    if !IS_EXIST_ARTICLE(agent, *FIGHTER_DIDDY_GENERATE_ARTICLE_DKBARREL) {
        if is_excute(agent) {
            ...
            ArticleModule::change_motion(boma, *FIGHTER_DIDDY_GENERATE_ARTICLE_DKBARREL, Hash40::new("win_3"), false, -1.0);
        }
    }
}
...
unsafe extern "C" fn ssbexo_diddy_win_3_wait_acmd(agent: &mut L2CAgentBase) {
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ArticleModule::change_motion(boma, *FIGHTER_DIDDY_GENERATE_ARTICLE_DKBARREL, Hash40::new("win_3_wait"), false, -1.0);
    }
}
