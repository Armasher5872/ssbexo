...
unsafe extern "C" fn diddy_special_lw_laugh_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_SPECIAL_LW | *FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK | *FIGHTER_LOG_MASK_FLAG_SHOOT) as u64, 0, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_LW as u32, 0);
    0.into()
}
...
unsafe extern "C" fn fun_710001d160(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_lw_banana_num_max = WorkModule::get_param_int(boma, hash40("param_special_lw"), hash40("special_lw_banana_num_max"));
    let interval_frame = WorkModule::get_int(boma, *FIGHTER_DIDDY_INSTANCE_WORK_ID_INT_SPECIAL_LW_INTERVAL_FRAME);
    if banana_count >= special_lw_banana_num_max {
        if interval_frame == 0 {
            WorkModule::on_flag(boma, *FIGHTER_DIDDY_STATUS_SPECIAL_LW_FLAG_PUTOUT_CONDITION_OK);
        }
    }
    0.into()
}
...
unsafe extern "C" fn diddy_special_lw_laugh_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if StatusModule::is_changing(boma) {
        if situation_kind != *SITUATION_KIND_GROUND {
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_lw_laugh"), -1.0, 1.0, 0.0, false, false);
        }
        else {
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_GROUND_STOP);
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_lw_laugh"), -1.0, 1.0, 0.0, false, false);
        }
    }
