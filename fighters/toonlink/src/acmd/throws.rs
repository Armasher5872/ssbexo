...
unsafe extern "C" fn ssbexo_toonlink_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        ArticleModule::change_motion(boma, *FIGHTER_TOONLINK_GENERATE_ARTICLE_HOOKSHOT_HAND, Hash40::new("back"), false, 0.0);
        WorkModule::on_flag(boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLAG_OFF_MAP_COLL_OFFSET);
    }
    frame(lua_state, 55.0);
    if is_excute(agent) {
        ArticleModule::remove_exist(boma, *FIGHTER_TOONLINK_GENERATE_ARTICLE_HOOKSHOT, ArticleOperationTarget(0));
        ArticleModule::remove_exist(boma, *FIGHTER_TOONLINK_GENERATE_ARTICLE_HOOKSHOT_HAND, ArticleOperationTarget(0));
    }
}
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_toonlink_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_toonlink_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_toonlink_pivot_grab_acmd, Low)
    .install()
    ;
}