...
unsafe extern "C" fn ssbexo_younglink_rapid_jab_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    loop {
        ssbexo_younglink_rapid_jab_sub_acmd(agent);
        wait(lua_state, 2.0);
        ssbexo_younglink_rapid_jab_sub_acmd(agent);
        wait(lua_state, 2.0);
        ssbexo_younglink_rapid_jab_sub_acmd(agent);
...
pub fn install() {
    ...
    .acmd("game_attack11", ssbexo_younglink_jab_1_acmd, Low)
    .acmd("game_attack100", ssbexo_younglink_rapid_jab_acmd, Low)
    .acmd("game_attackdash", ssbexo_younglink_dash_attack_acmd, Low)
    .install()
    ;
}