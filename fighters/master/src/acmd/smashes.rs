...
unsafe extern "C" fn ssbexo_master_forward_smash_acmd(agent: &mut L2CAgentBase) {
    ...
    sv_animcmd::execute(agent.lua_state_agent, 14.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    .game_acmd("game_attacks4", ssbexo_master_forward_smash_acmd, Low)
    .game_acmd("game_attacklw4", ssbexo_master_down_smash_acmd, Low)
    .install()
    ;
}