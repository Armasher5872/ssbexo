...
unsafe extern "C" fn ssbexo_inkling_dash_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        MotionModule::set_rate(boma, 1.125);
    }
    frame(lua_state, 1.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_inkling_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_inkling_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_inkling_pivot_grab_acmd, Low)
    .install()
    ;
}