...
unsafe extern "C" fn inkling_inkbullet_hit_main_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let get_touch_pos = GroundModule::get_touch_pos(boma, *GROUND_TOUCH_FLAG_DOWN as u32);
    ...
    MotionModule::change_motion(boma, Hash40::new("clash"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn inkling_inkbullet_hit_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let get_speed_x = {
        ...
        sv_kinetic_energy::get_speed_x(lua_state)
    };
    let get_speed_y = {
        weapon.clear_lua_stack();
        lua_args!(weapon, *WEAPON_KINETIC_ENERGY_RESERVE_ID_NORMAL);
        sv_kinetic_energy::get_speed_y(lua_state)
    };
    let r = WorkModule::get_float(boma, *WEAPON_INKLING_INKBULLET_INSTANCE_WORK_ID_FLOAT_R);
    let g = WorkModule::get_float(boma, *WEAPON_INKLING_INKBULLET_INSTANCE_WORK_ID_FLOAT_G);
    let b = WorkModule::get_float(boma, *WEAPON_INKLING_INKBULLET_INSTANCE_WORK_ID_FLOAT_B);
    let pos = PostureModule::pos(boma);
    let scale = PostureModule::scale(boma);
    let get_touch_pos_up = GroundModule::get_touch_pos(boma, *GROUND_TOUCH_FLAG_UP as u32);
    let get_touch_normal_up = GroundModule::get_touch_normal(boma, *GROUND_TOUCH_FLAG_UP as u32);
    let get_touch_pos_down = GroundModule::get_touch_pos(boma, *GROUND_TOUCH_FLAG_DOWN as u32);
    let get_touch_normal_down = GroundModule::get_touch_normal(boma, *GROUND_TOUCH_FLAG_DOWN as u32);
    let get_touch_pos_left = GroundModule::get_touch_pos(boma, *GROUND_TOUCH_FLAG_LEFT as u32);
    let get_touch_normal_left = GroundModule::get_touch_normal(boma, *GROUND_TOUCH_FLAG_LEFT as u32);
    let get_touch_pos_right = GroundModule::get_touch_pos(boma, *GROUND_TOUCH_FLAG_RIGHT as u32);
    let get_touch_normal_right = GroundModule::get_touch_normal(boma, *GROUND_TOUCH_FLAG_RIGHT as u32);
    let pos_x = (*pos).x;
    let pos_y = (*pos).y;
    let pos_z = (*pos).z;
    let mut x_pos = pos_x;
    ...
    if WorkModule::is_flag(boma, *WEAPON_INKLING_INKBULLET_INSTANCE_WORK_ID_FLAG_HIT) {
        EffectModule::req(boma, Hash40::new_raw(0x17f8dcd76c), &Vector3f{x: pos_x, y: pos_y, z: pos_z}, &Vector3f::zero(), 1.0, 0u32, -1, false, 0);
        EffectModule::set_rgb_partial_last(boma, r, g, b);
    }
    else {
        if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_UP as u32) {
        ...
        EffectModule::req(boma, Hash40::new_raw(0x1a72de490f), &Vector3f{x: x_pos, y: y_pos, z: pos_z}, &Vector3f{x: (x_touch).atan(), y: (y_touch).atan(), z: pos_z}, 1.0, 0u32, -1, false, 0);
        EffectModule::set_rgb_partial_last(boma, r, g, b);
        SoundModule::play_se(boma, Hash40::new_raw(0x16fe772137), true, false, false, false, enSEType(0));
...
unsafe extern "C" fn fun_71000369c0(weapon: &mut L2CWeaponCommon, x_pos: L2CValue, y_pos: L2CValue, z_pos: L2CValue, scale_1: L2CValue, scale_2: L2CValue) {
    let boma = weapon.module_accessor;
    let paint_pos_rand = WorkModule::get_param_float(boma, hash40("param_inkbullet"),  hash40("paint_pos_rand"));
    ...
    LinkModule::send_event_parents_struct(boma, *WEAPON_LINK_NO_CONSTRAINT, link_event);
