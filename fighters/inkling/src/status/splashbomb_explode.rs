...
unsafe extern "C" fn inkling_splashbomb_explode_main_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let get_touch_normal = GroundModule::get_touch_normal(boma, *GROUND_TOUCH_FLAG_DOWN as u32);
    ...
    MotionModule::change_motion(boma, Hash40::new("explode"), 0.0, 1.0, true, 0.0, false, false);
    VisibilityModule::set_whole(boma, false);
    EffectModule::detach_all(boma, 5);
    if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_DOWN as u32) {
        EffectModule::req(boma, Hash40::new_raw(0x18c19c7ee8), &Vector3f{x: get_touch_pos.x, y: get_touch_pos.y, z: (*pos).z}, &Vector3f{x: (get_touch_normal.x).atan(), y: (get_touch_normal.y).atan().to_degrees(), z: (*pos).z}, 1.0, 0u32, -1, false, 0);
...
unsafe extern "C" fn inkling_splashbomb_explode_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let boma = weapon.module_accessor;
    let float_attack = WorkModule::get_float(boma, *WEAPON_INKLING_SPLASHBOMB_INSTANCE_WORK_ID_FLOAT_ATTACK);
    let float_ink = WorkModule::get_float(boma, *WEAPON_INKLING_SPLASHBOMB_INSTANCE_WORK_ID_FLOAT_INK);
    AttackModule::set_power(boma, 0, float_attack, false);
    AttackModule::set_ink_value(boma, 0, float_ink);
