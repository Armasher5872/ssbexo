...
//Standing Grab ACMD
unsafe extern "C" fn ssbexo_richter_grab_acmd(agent: &mut L2CAgentBase) {
...
//Dash Grab ACMD
unsafe extern "C" fn ssbexo_richter_dash_grab_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
...
//Pivot Grab ACMD
unsafe extern "C" fn ssbexo_richter_pivot_grab_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
        FighterAreaModuleImpl::enable_fix_jostle_area(boma, 6.5, 6.0);
        GrabModule::set_rebound(boma, true);
        MotionModule::set_rate(boma, 1.273);
    }
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_richter_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_richter_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_richter_pivot_grab_acmd, Low)
    .install()
    ;
}