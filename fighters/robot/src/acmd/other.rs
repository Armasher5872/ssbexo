...
unsafe extern "C" fn ssbexo_robot_turn_dash_effect(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        ...
        if WorkModule::is_flag(agent.module_accessor, *FIGHTER_ROBOT_INSTANCE_WORK_ID_FLAG_CAN_SNAKE) {
            EFFECT_FOLLOW(agent, Hash40::new("sys_damage_fire_fly"), Hash40::new("knee1"), 1.2, 0, 0, 0, 0, 0, 1, true);
            EFFECT_FOLLOW(agent, Hash40::new("sys_damage_spark"), Hash40::new("knee1"), 1.2, 0, 0, 90, 0, 0, 1, true);
        }
    }
    frame(agent.lua_state_agent, 3.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    .game_acmd("game_turndash", ssbexo_robot_turn_dash_acmd, Low)
    .effect_acmd("effect_turndash", ssbexo_robot_turn_dash_effect, Low)
    .game_acmd("game_turnrun", ssbexo_robot_turn_run_acmd, Low)
    .install()
    ;
}