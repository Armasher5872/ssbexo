...
unsafe extern "C" fn springtrap_special_n_charge_loop_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    let effect = EffectModule::req_follow(boma, Hash40::new("springtrap_vector"), Hash40::new("top"), &Vector3f{x: 0.0, y: 10.0, z: 0.0}, &Vector3f::zero(), 1.0, true, 0, 0, 0, 0, 0, true, true);
...
unsafe extern "C" fn springtrap_special_n_charge_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ArticleModule::is_exist(boma, *FIGHTER_GANON_GENERATE_ARTICLE_SWORD) {
        ...
        LinkModule::set_constraint_translate_offset(axe_boma, &Vector3f::zero());
        LinkModule::set_constraint_rot_offset(axe_boma, &Vector3f::zero());
    }
