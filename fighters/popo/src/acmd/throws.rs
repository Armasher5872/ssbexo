...
//Standing Grab ACMD
unsafe extern "C" fn ssbexo_popo_grab_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
...
//Dash Grab ACMD
unsafe extern "C" fn ssbexo_popo_dash_grab_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        GrabModule::set_rebound(agent.module_accessor, true);
    }
...
//Pivot Grab
unsafe extern "C" fn ssbexo_popo_pivot_grab_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        GrabModule::set_rebound(agent.module_accessor, true);
    }
...
pub fn install() {
    ...
    .game_acmd("game_catch", ssbexo_popo_grab_acmd, Low)
    .game_acmd("game_catchdash", ssbexo_popo_dash_grab_acmd, Low)
    .game_acmd("game_catchturn", ssbexo_popo_pivot_grab_acmd, Low)
    .install()
    ;
}