...
unsafe extern "C" fn ssbexo_tantan_punch1_attack_short_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ATTACK(agent, 0, 0, Hash40::new("have"), 8.0, 361, knockback_growth, 0, 50, 2.9, 3.1, 0.5, 0.0, Some(1.0), Some(0.5), Some(0.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_F, false, -2.5, 0.0, 0, true, false, false, true, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_TANTAN_PUNCH01, *ATTACK_REGION_PUNCH);
...
pub fn install() {
    ...
    .game_acmd("game_attackshortstartl1", ssbexo_tantan_attackshortstartl1_acmd, Low)
    .install()
    ;
    Agent::new("tantan_punch1")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_attackshort", ssbexo_tantan_punch1_attack_short_acmd, Low)
    .install()
    ;
    Agent::new("tantan_punch2")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_attackshort", ssbexo_tantan_punch2_attack_short_acmd, Low)
    .install()
    ;
    Agent::new("tantan_punch3")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_attackshort", ssbexo_tantan_punch3_attack_short_acmd, Low)
    .install()
    ;
    Agent::new("tantan_ring")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_attackfly", ssbexo_tantan_ring_attack_fly_acmd, Low)
    .install()
    ;
}