...
unsafe extern "C" fn edge_run_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    edge_status_run_sub(fighter);
    fighter.sub_shift_status_main(L2CValue::Ptr(L2CFighterCommon_bind_address_call_status_Run_Main as *const () as _))
}

unsafe extern "C" fn edge_status_run_sub(fighter: &mut L2CFighterCommon) {
    let prev_status_kind = fighter.global_table[PREV_STATUS_KIND].get_i32();
    let boma = fighter.module_accessor;
    let start_frame = if [*FIGHTER_STATUS_KIND_DASH, *FIGHTER_STATUS_KIND_TURN_DASH].contains(&prev_status_kind) {WorkModule::get_float(boma, *FIGHTER_STATUS_RUN_WORK_FLOAT_START_FRAME)} else {0.0};
    let motion = if WorkModule::is_flag(boma, *FIGHTER_EDGE_INSTANCE_WORK_ID_FLAG_ONE_WINGED_ACTIVATED) {"run_wing"} else {"run"};
    ...
    MotionModule::change_motion(boma, Hash40::new(motion), start_frame, 1.0, false, 0.0, false, false);
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_RUN, edge_run_main_status)
    .install()
    ;
}