...
unsafe extern "C" fn ssbexo_edge_scintilla_attack_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    let attack_ratio = WorkModule::get_float(boma, *WEAPON_EDGE_FLASH_INSTANCE_WORK_ID_FLOAT_ATTACK_RATIO);
    if !WorkModule::is_flag(boma, *WEAPON_EDGE_FLASH_INSTANCE_WORK_ID_FLAG_HIT) {
        if is_excute(agent) {
    ...
    if is_excute(agent) {
        ...
        if is_excute(agent) {
            ...
            ATK_LERP_RATIO(agent, attack_ratio);
        }
    }
    frame(lua_state, 15.0);
    if is_excute(agent) {
