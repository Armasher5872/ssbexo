...
//Side Special Air Loop Pre Status
unsafe extern "C" fn wario_special_s_air_loop_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
...
//Side Special Air Loop Init Status
unsafe extern "C" fn wario_special_s_air_loop_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    let lr = PostureModule::lr(boma);
    if fighter.global_table[PREV_STATUS_KIND].get_i32() == *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_JUMPSQUAT {
...
//Side Special Air Loop Main Status
unsafe extern "C" fn wario_special_s_air_loop_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("special_s_air_loop"), 0.0, 1.0, false, 0.0, false, false);
    fighter.sub_shift_status_main(L2CValue::Ptr(wario_special_s_air_loop_main_loop as *const () as _))
}

unsafe extern "C" fn wario_special_s_air_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
//Side Special Air Loop Exec Status
unsafe extern "C" fn wario_special_s_air_loop_exec_status(_fighter: &mut L2CFighterCommon) -> L2CValue {
    0.into()
}

//Side Special Air Loop End Status
...
//Side Special Air Loop Exit Status
...
pub fn install() {
    ...
    .status(Pre, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_pre_status)
    .status(Init, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_init_status)
    .status(Main, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_main_status)
    .status(Exec, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_exec_status)
    .status(End, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_end_status)
    .status(Exit, *FIGHTER_WARIO_STATUS_KIND_SPECIAL_S_AIR_LOOP, wario_special_s_air_loop_exit_status)
    .install()
    ;
}