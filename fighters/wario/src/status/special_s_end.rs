...
unsafe extern "C" fn wario_special_s_end_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_AIR {
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION);
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_s_end"), -1.0, 1.0, 0.0, false, false);
        }
        if situation_kind == *SITUATION_KIND_AIR
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_s_air_end"), -1.0, 1.0, 0.0, false, false);
        }
    }
