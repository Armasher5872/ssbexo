...
unsafe extern "C" fn ssbexo_wario_neutral_special_down_throw_land_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        let multiplier = WorkModule::get_float(boma, *FIGHTER_WARIO_INSTANCE_WORK_ID_FLOAT_SPECIAL_N_PILEDRIVER_MULTIPLIER);
...
pub fn install() {
    ...
    .acmd("game_specialnthrowlwland", ssbexo_wario_neutral_special_down_throw_land_acmd, Low)
    .acmd("effect_specialnthrowlwland", ssbexo_wario_neutral_special_down_throw_land_effect, Low)
    .acmd("sound_specialnthrowlwland", ssbexo_wario_neutral_special_down_throw_land_sound, Low)
    .acmd("expression_specialnthrowlwland", ssbexo_wario_neutral_special_down_throw_land_expression, Low)
    .install()
    ;
}