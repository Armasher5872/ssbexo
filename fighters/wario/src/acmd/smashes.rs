...
unsafe extern "C" fn ssbexo_wario_down_smash_acmd(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 17.0);
    if is_excute(agent) {
        ATTACK(agent, 2, 0, Hash40::new("top"), 7.4, 39, 46, 0, 72, 8.0, 0.0, 4.0, -10.0, Some(0.0), Some(4.0), Some(11.5), 1.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, false, *COLLISION_SITUATION_MASK_G, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_KICK, *ATTACK_REGION_NONE);
    }
    frame(lua_state, 18.0);
    if is_excute(agent) {
...
unsafe extern "C" fn ssbexo_wario_down_smash_effect(agent: &mut L2CAgentBase) {
    ...
    if get_value_float(lua_state, *SO_VAR_FLOAT_LR) < 0.0 {
        if is_excute(agent) {
            ...
            let wario_quake = EffectModule::req_follow(boma, Hash40::new("wario_quake"), Hash40::new("top"), &Vector3f{x: 5.0, y: 0.0, z: -2.5}, &Vector3f::zero(), 0.5, true, 0, 0, 0, 0, 0, true, true);
            EffectModule::set_scale(boma, wario_quake as u32, &Vector3f{x: 1.0, y: 0.5, z: 1.35});
            EffectModule::set_rate(boma, wario_quake as u32, 2.0);
        }
    }
    else {
        if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attacks4", ssbexo_wario_forward_smash_acmd, Low)
    ...
    .acmd("sound_attacklw4", ssbexo_wario_down_smash_sound, Low)
    .acmd("expression_attacklw4", ssbexo_wario_down_smash_expression, Low)
    .install()
    ;
}