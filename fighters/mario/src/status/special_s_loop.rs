...
unsafe extern "C" fn mario_special_s_loop_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("special_s_loop"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn mario_special_s_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_s_degree = WorkModule::get_float(boma, *FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_SPECIAL_S_DEGREE);
    let max_degree = 90.0;
    ...
    if WorkModule::is_enable_transition_term_group(boma, *FIGHTER_STATUS_TRANSITION_GROUP_CHK_AIR_JUMP_AERIAL) {
    ...
    if special_s_degree > -max_degree {
        WorkModule::set_float(boma, special_s_degree-change_degree_per_frame, *FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_SPECIAL_S_DEGREE);
    }
    change_angle(boma, special_s_degree, max_degree, "special_s_loop_down", "special_s_loop_down");
...
unsafe extern "C" fn mario_special_s_loop_exec_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let get_sum_speed_y = KineticModule::get_sum_speed_y(boma, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    let lr = PostureModule::lr(boma);
    let attack_frame = WorkModule::get_float(boma, *FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_SPECIAL_S_ATTACK_FRAME);
    let gravity = if get_sum_speed_y > -1.5 {0.18} else {0.0242};
    if stick_x*lr < -0.2 {
    ...
    if attack_frame > 32.0 {
        ...
        WorkModule::add_float(boma, 1.0, *FIGHTER_MARIO_INSTANCE_WORK_ID_FLOAT_SPECIAL_S_ATTACK_FRAME);
    }
    0.into()
}
