...
unsafe extern "C" fn ssbexo_krool_grounded_down_special_launch_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if ArticleModule::is_exist(boma, *FIGHTER_KROOL_GENERATE_ARTICLE_BLUNDERBUSS) {
            ArticleModule::change_motion(boma, *FIGHTER_KROOL_GENERATE_ARTICLE_BLUNDERBUSS, Hash40::new("special_lw_launch"), false, -1.0);
        }
    }
    frame(lua_state, 5.0);
    if is_excute(agent) {
        WorkModule::on_flag(boma, *FIGHTER_KROOL_INSTANCE_WORK_ID_FLAG_SPECIAL_LW_JUMP);
    }
    frame(lua_state, 6.0);
    if is_excute(agent) {
        let launch_angle = WorkModule::get_int64(boma, *FIGHTER_KROOL_INSTANCE_WORK_ID_INT_SPECIAL_LW_ANGLE) as u64;
        let charge = WorkModule::get_float(boma, *FIGHTER_KROOL_INSTANCE_WORK_ID_FLOAT_SPECIAL_LW_CHARGE);
...
unsafe extern "C" fn ssbexo_krool_aerial_down_special_launch_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if ArticleModule::is_exist(boma, *FIGHTER_KROOL_GENERATE_ARTICLE_BLUNDERBUSS) {
            ArticleModule::change_motion(boma, *FIGHTER_KROOL_GENERATE_ARTICLE_BLUNDERBUSS, Hash40::new("special_air_lw_launch"), false, -1.0);
        }
    }
    frame(lua_state, 5.0);
    if is_excute(agent) {
