...
unsafe extern "C" fn krool_on_start(fighter: &mut L2CFighterCommon) {
    ...
    fighter.global_table[STATUS_END_CONTROL].assign(&L2CValue::Ptr(krool_end_control as *const () as _));
}

pub fn install() {
    Agent::new("krool")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .on_start(krool_on_start)
    .install()
    ;
}