...
unsafe extern "C" fn ssbexo_pickel_nair_acmd(agent: &mut L2CAgentBase) {
    ...
    let situation_kind = WorkModule::get_int(boma, *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_SITUATION_KIND);
...
unsafe extern "C" fn ssbexo_pickel_uair_acmd(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 2.0);
    if is_excute(agent) {
        WorkModule::on_flag(boma, *FIGHTER_PICKEL_INSTANCE_WORK_ID_FLAG_ATTACK_AIR_HI_ENABLE_LANDING);
    ...
    if is_excute(agent) {
        ...
        if [wood, stone, iron, gold, diamond].contains(&material_kind) {
            ...
            ATTACK(agent, 0, 0, Hash40::new("armr"), 3.0, 71, 76, 0, 48, 4.4, 0.6, 0.4, 0.0, None, None, None, 1.0, 1.3, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_F, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_PUNCH, *ATTACK_REGION_PUNCH);
        }
    }
    wait(lua_state, 5.0);
    if is_excute(agent) {
        AttackModule::clear_all(boma);
        if material_kind == gold {
            MotionModule::set_rate(boma, 0.571);
            MotionModule::set_rate_partial(boma, *FIGHTER_MOTION_PART_SET_KIND_UPPER_BODY, 0.571);
        }
        else {
            MotionModule::set_rate(boma, 0.444);
            MotionModule::set_rate_partial(boma, *FIGHTER_MOTION_PART_SET_KIND_UPPER_BODY, 0.444);
        }
    }
...
pub fn install() {
    ...
    .acmd("game_attackairn", ssbexo_pickel_nair_acmd, Low)
    .acmd("game_attackairhi", ssbexo_pickel_uair_acmd, Low)
    .install()
    ;
}