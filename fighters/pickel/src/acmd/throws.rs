...
unsafe extern "C" fn ssbexo_pickel_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        ArticleModule::generate_article(boma, *FIGHTER_PICKEL_GENERATE_ARTICLE_FISHINGROD, false, 0);
    ...
    if is_excute(agent) {
        ArticleModule::set_visibility(boma, *FIGHTER_PICKEL_GENERATE_ARTICLE_FISHINGROD, Hash40::new("rod"), Hash40::new("rod_cast"), ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_pickel_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_pickel_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_pickel_pivot_grab_acmd, Low)
    .install()
    ;
}