...
unsafe extern "C" fn dolly_super_special_2_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let speed_mul = WorkModule::get_param_float(boma, hash40("param_super_special2") as u64, hash40("speed_mul") as u64);
    MotionModule::change_motion(boma, Hash40::new("super_special2_start"), 0.0, 1.0, false, 0.0, false, false);
    ...
    WorkModule::set_int(boma, *BATTLE_OBJECT_CATEGORY_INVALID, *FIGHTER_DOLLY_STATUS_SUPER_SPECIAL2_WORK_INT_TARGET_OBJECT_CATEGORY);
    WorkModule::set_int(boma, *BATTLE_OBJECT_ID_INVALID, *FIGHTER_DOLLY_STATUS_SUPER_SPECIAL2_WORK_INT_TARGET_OBJECT_ID);
    WorkModule::set_int(boma, *BATTLE_OBJECT_ID_INVALID, *FIGHTER_DOLLY_STATUS_SUPER_SPECIAL2_WORK_INT_HIT_OBJECT_ID);
...
unsafe extern "C" fn dolly_super_special_2_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !fighter.sub_transition_group_check_air_cliff().get_bool() {
        ...
        if MotionModule::is_end(boma) {
            ...
            if !WorkModule::is_flag(boma, *FIGHTER_DOLLY_STATUS_SUPER_SPECIAL2_WORK_FLAG_HIT) {
                return 0.into();
            }
            WorkModule::set_int(boma, *BATTLE_OBJECT_ID_INVALID, *FIGHTER_DOLLY_STATUS_SUPER_SPECIAL2_WORK_INT_HIT_OBJECT_ID);
        }
...
unsafe extern "C" fn dolly_super_special_2_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[STATUS_KIND].get_i32() != *FIGHTER_DOLLY_STATUS_KIND_SUPER_SPECIAL2_BLOW {
        ArticleModule::remove_exist(boma, *FIGHTER_DOLLY_GENERATE_ARTICLE_FIRE, ArticleOperationTarget(0));
        EffectModule::kill_kind(boma, Hash40::new("dolly_buster_punch"), true, true);
        EffectModule::kill_kind(boma, Hash40::new("dolly_buster_dash"), true, true);
    }
    WorkModule::off_flag(boma, *FIGHTER_DOLLY_INSTANCE_WORK_ID_FLAG_FEINTED);
    0.into()
}
