...
unsafe extern "C" fn dolly_special_n_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::off_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE);
    WorkModule::off_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE_DONE);
...
unsafe extern "C" fn dolly_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !MotionModule::is_end(boma) {
        if WorkModule::is_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE) {
            ArticleModule::generate_article_enable(boma, *FIGHTER_DOLLY_GENERATE_ARTICLE_WAVE, false, -1);
            if situation_kind != *SITUATION_KIND_GROUND {
                if !WorkModule::is_flag(boma, *FIGHTER_DOLLY_INSTANCE_WORK_ID_FLAG_SPECIAL_N_HOP_DONE) {
                    let hop_speed_y = WorkModule::get_param_float(boma, hash40("param_special_n"), hash40("hop_speed_y"));
                    let gravity_accel = WorkModule::get_param_float(boma, hash40("param_special_n"), hash40("gravity_accel"));
                    let gravity_max = WorkModule::get_param_float(boma, hash40("param_special_n"), hash40("gravity_max"));
                    ...
                    WorkModule::on_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE_DONE);
                    WorkModule::on_flag(boma, *FIGHTER_DOLLY_INSTANCE_WORK_ID_FLAG_SPECIAL_N_HOP_DONE);
                }
            }
            WorkModule::off_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE);
        }
        if WorkModule::is_flag(boma, *FIGHTER_DOLLY_STATUS_SPECIAL_N_WORK_FLAG_GENERATE_DONE) {
            if situation_kind != *SITUATION_KIND_GROUND {
