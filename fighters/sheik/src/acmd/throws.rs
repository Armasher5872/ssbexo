...
unsafe extern "C" fn ssbexo_sheik_up_throw_acmd(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 2.0, 85, 80, 0, 60, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
...
pub fn install() {
    ...
    .acmd("game_catch", ssbexo_sheik_grab_acmd, Low)
    .acmd("game_catchdash", ssbexo_sheik_dash_grab_acmd, Low)
    .acmd("game_catchturn", ssbexo_sheik_pivot_grab_acmd, Low)
    .acmd("game_throwhi", ssbexo_sheik_up_throw_acmd, Low)
    .install()
    ;
}