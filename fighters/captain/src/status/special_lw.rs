...
unsafe extern "C" fn captain_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(boma, smash::app::SituationKind(*SITUATION_KIND_NONE), *FIGHTER_KINETIC_TYPE_FALL, *GROUND_CORRECT_KIND_KEEP as u32, smash::app::GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLOAT, 0);
...
unsafe extern "C" fn captain_special_lw_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let hit_reduct_count = WorkModule::get_param_int(boma, hash40("param_special_lw"), hash40("hit_reduct_count"));
    ...
    WorkModule::off_flag(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_KICK_CLIFF_CHECK);
    WorkModule::off_flag(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_KICK_WALL_CHECK);
    WorkModule::off_flag(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_KICK_FALL_ONOFF);
    WorkModule::set_int(boma, hit_reduct_count, *FIGHTER_CAPTAIN_STATUS_WORK_ID_INT_FALCON_KICK_REDUCTION_LEFT);
    WorkModule::set_float(boma, 1.0, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLOAT_FALCON_KICK_SPEED_COEFFICIENT);
    if situation_kind != *SITUATION_KIND_GROUND {
        WorkModule::set_int(boma, *SITUATION_KIND_AIR, *FIGHTER_CAPTAIN_STATUS_WORK_ID_INT_FALCON_KICK_START_SITUATION);
        MotionModule::change_motion(boma, Hash40::new("special_air_lw"), 0.0, 1.0, false, 0.0, false, false);
        sv_kinetic_energy!(reset_energy, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, *ENERGY_STOP_RESET_TYPE_AIR, get_sum_speed_x*0.5, 0.0, 0.0, 0.0, 0.0);
        sv_kinetic_energy!(reset_energy, fighter, *FIGHTER_KINETIC_ENERGY_ID_CONTROL, *ENERGY_CONTROLLER_RESET_TYPE_FALL_ADJUST, get_sum_speed_x*0.5, 0.0, 0.0, 0.0, 0.0);
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
        fighter.set_situation(SITUATION_KIND_AIR.into());
        GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
    }
    else {
        WorkModule::set_int(boma, *SITUATION_KIND_GROUND, *FIGHTER_CAPTAIN_STATUS_WORK_ID_INT_FALCON_KICK_START_SITUATION);
...
unsafe extern "C" fn captain_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let kick_start_situation = WorkModule::get_int(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_INT_FALCON_KICK_START_SITUATION);
    ...
    if kick_start_situation != *SITUATION_KIND_GROUND {
        if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_DOWN as u32) {
            if WorkModule::is_flag(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_KICK_WALL_CHECK) {
                fighter.change_status(FIGHTER_CAPTAIN_STATUS_KIND_SPECIAL_LW_END.into(), false.into());
            }
        }
    }
    else {
        if GroundModule::is_touch(boma, touch as u32) {
            if WorkModule::is_flag(boma, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_KICK_WALL_CHECK) {
    ...
    if situation_kind == *SITUATION_KIND_AIR {
        if touch == *GROUND_TOUCH_FLAG_LEFT && GroundModule::is_touch(boma, touch as u32) {
        ...
        if touch == *GROUND_TOUCH_FLAG_RIGHT && GroundModule::is_touch(boma, touch as u32) {
