...
unsafe extern "C" fn ssbexo_rockman_dash_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 2.9, 0.0, 6.6, 4.5, Some(0.0), Some(6.6), Some(10.6), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
        GrabModule::set_constraint(agent.module_accessor, 0, true);
    }
    game_CaptureCutCommon(agent);
    wait(agent.lua_state_agent, 2.0);
...
pub fn install() {
    ...
    .game_acmd("game_catch", ssbexo_rockman_grab_acmd, Low)
    .game_acmd("game_catchdash", ssbexo_rockman_dash_grab_acmd, Low)
    .game_acmd("game_catchturn", ssbexo_rockman_pivot_grab_acmd, Low)
    .install()
    ;
}