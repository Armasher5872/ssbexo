...
unsafe extern "C" fn ssbexo_purin_dash_attack_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        WorkModule::set_float(boma, 0.2, *FIGHTER_INSTANCE_WORK_ID_FLOAT_ATTACK_DASH_FALL_SPEED_Y_MUL);
    }
    frame(lua_state, 5.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_appealhir", ssbexo_purin_up_taunt_acmd, Low)
    .acmd("game_appealhil", ssbexo_purin_up_taunt_acmd, Low)
    .acmd("game_attackdash", ssbexo_purin_dash_attack_acmd, Low)
    .acmd("game_attackairdash", ssbexo_purin_aerial_dash_attack_acmd, Low)
    .acmd("effect_attackairdash", ssbexo_purin_aerial_dash_attack_effect, Low)
    .acmd("sound_attackairdash", ssbexo_purin_aerial_dash_attack_sound, Low)
    .acmd("expression_attackairdash", ssbexo_purin_aerial_dash_attack_expression, Low)
    .install()
    ;
}