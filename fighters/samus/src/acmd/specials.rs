...
unsafe extern "C" fn ssbexo_samus_homing_missile_homing_acmd(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        ATTACK(agent, 0, 0, Hash40::new("top"), 10.0, 60, 18, 0, 40, 2.4, 0.0, 0.0, 0.0, None, None, None, 1.0, 1.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_SPEED, false, 1, 0.0, 0, true, false, false, false, false, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_fire"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_FIRE, *ATTACK_REGION_OBJECT);
...
pub fn install() {
    ...
    .game_acmd("game_specialhi", ssbexo_samus_grounded_up_special_acmd, Low)
    .game_acmd("game_specialairhi", ssbexo_samus_aerial_up_special_acmd, Low)
    .install()
    ;
    Agent::new("samus_cshot")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_shoot", ssbexo_samus_charge_shot_shoot_acmd, Low)
    .install()
    ;
    Agent::new("samus_missile")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_homing", ssbexo_samus_homing_missile_homing_acmd, Low)
    .effect_acmd("effect_homing", ssbexo_samus_homing_missile_homing_effect, Low)
    .install()
    ;
    Agent::new("samus_supermissile")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .game_acmd("game_ready", ssbexo_samus_super_missile_ready_acmd, Low)
    .game_acmd("game_straight", ssbexo_samus_super_missile_straight_acmd, Low)
    .install()
    ;
}