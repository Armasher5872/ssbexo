...
unsafe extern "C" fn ssbexo_samus_final_smash_acmd(agent: &mut L2CAgentBase) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL_CHARGE) {
        if !IS_EXIST_ARTICLE(agent, *FIGHTER_SAMUS_GENERATE_ARTICLE_LASER2) {
            if is_excute(agent) {
                ArticleModule::shoot(boma, *FIGHTER_SAMUS_GENERATE_ARTICLE_LASER, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL), false);
                ...
                WorkModule::set_float(boma, -14.0, *FIGHTER_SAMUS_STATUS_FINAL_WORK_FLOAT_OY);
                ArticleModule::generate_article(boma, *FIGHTER_SAMUS_GENERATE_ARTICLE_LASER2, false, -1);
                ArticleModule::shoot(boma, *FIGHTER_SAMUS_GENERATE_ARTICLE_LASER2, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL), false);
            }
        }
        if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_finalair", ssbexo_samus_final_smash_acmd, Low)
    .install()
    ;
    Agent::new("samus_laser")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .acmd("game_shoot", ssbexo_samus_laser_shoot_acmd, Low)
    .install()
    ;
    Agent::new("samus_laser2")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .acmd("game_lower", ssbexo_samus_laser_2_lower_acmd, Low)
    .acmd("game_upper", ssbexo_samus_laser_2_upper_acmd, Low)
    .install()
    ;
}