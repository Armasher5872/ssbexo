...
unsafe extern "C" fn ssbexo_metaknight_down_special_start_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        ShieldModule::set_status(boma, *FIGHTER_METAKNIGHT_SHIELD_KIND_SPECIAL_LW_GUARD, ShieldStatus(*SHIELD_STATUS_NORMAL), *FIGHTER_METAKNIGHT_SHIELD_GROUP_KIND_SPECIAL_LW_GUARD);
    }
    frame(lua_state, 25.0);
    if is_excute(agent) {
        WorkModule::off_flag(boma, *FIGHTER_METAKNIGHT_STATUS_SPECIAL_LW_START_FLAG_VIS_OFF);
        ArticleModule::change_motion(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, Hash40::new("special_lw_end"), false, -1.0);
        ShieldModule::set_status(boma, *FIGHTER_METAKNIGHT_SHIELD_KIND_SPECIAL_LW_GUARD, ShieldStatus(*SHIELD_STATUS_NONE), *FIGHTER_METAKNIGHT_SHIELD_GROUP_KIND_SPECIAL_LW_GUARD);
    }
...
pub fn install() {
    ...
    .acmd("game_speciallwstart", ssbexo_metaknight_down_special_start_acmd, Low)
    .acmd("game_specialairlwstart", ssbexo_metaknight_down_special_start_acmd, Low)
    .acmd("effect_speciallwstart", ssbexo_metaknight_down_special_start_effect, Low)
    .acmd("effect_specialairlwstart", ssbexo_metaknight_down_special_start_effect, Low)
    .acmd("sound_speciallwstart", ssbexo_metaknight_down_special_start_sound, Low)
    .acmd("sound_specialairlwstart", ssbexo_metaknight_down_special_start_sound, Low)
    .acmd("expression_speciallwstart", ssbexo_metaknight_down_special_start_expression, Low)
    .acmd("expression_specialairlwstart", ssbexo_metaknight_down_special_start_expression, Low)
    .install()
    ;
}