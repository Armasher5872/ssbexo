...
pub unsafe extern "C" fn metaknight_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    if WorkModule::is_flag(boma, *FIGHTER_METAKNIGHT_INSTANCE_WORK_ID_FLAG_SPECIAL_LW_POWER) {
        StatusModule::set_status_kind_interrupt(boma, *FIGHTER_METAKNIGHT_STATUS_KIND_SPECIAL_LW_END);
        return 1.into();
    }
    else {
...
unsafe extern "C" fn metaknight_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_AIR {
            ...
            if !WorkModule::is_flag(boma, *FIGHTER_METAKNIGHT_STATUS_SPECIAL_LW_START_FLAG_VIS_OFF) {
                ArticleModule::set_visibility_whole(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, true, ArticleOperationTarget(0));
                if current_frame >= 25.0 {
                    ArticleModule::change_motion(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, Hash40::new("special_lw_end"), true, -1.0);
                }
                else {
                    ArticleModule::change_motion(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, Hash40::new("special_lw_start"), true, -1.0);
                }
            }
            else {
        ...
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            ...
            if current_frame >= 25.0 {
                ArticleModule::change_motion(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, Hash40::new("special_air_lw_end"), true, -1.0);
            }
            else {
                ArticleModule::change_motion(boma, *FIGHTER_METAKNIGHT_GENERATE_ARTICLE_MANTLE, Hash40::new("special_air_lw_start"), true, -1.0);
            }
        }
    }
