...
unsafe extern "C" fn kirby_special_hi_2_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::set_int(boma, 0, *FIGHTER_KIRBY_STATUS_WORK_ID_INT_FINALCUTTER_METEOR_HIT_NUM);
    fun_71001a6740(fighter);
    WorkModule::off_flag(boma, *FIGHTER_KIRBY_STATUS_WORK_ID_FLAG_FINALCUTTER_MOT_FRAME_INHERIT);
    WorkModule::set_int64(boma, hash40("special_hi2") as i64, *FIGHTER_KIRBY_STATUS_WORK_ID_INT_FINALCUTTER_GROUND_MOT);
    WorkModule::set_int64(boma, hash40("special_air_hi2") as i64, *FIGHTER_KIRBY_STATUS_WORK_ID_INT_FINALCUTTER_AIR_MOT);
...
unsafe extern "C" fn kirby_special_hi_2_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_KIRBY_STATUS_WORK_ID_FLAG_FINALCUTTER_CANCEL) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_KIRBY_INSTANCE_WORK_ID_FLAG_SPECIAL_HI_INPUT) {
...
unsafe extern "C" fn kirby_special_hi_2_exit_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    if ![*FIGHTER_KIRBY_STATUS_KIND_SPECIAL_HI3, *FIGHTER_KIRBY_STATUS_KIND_SPECIAL_HI4, *FIGHTER_KIRBY_STATUS_KIND_SPECIAL_HI5].contains(&status_kind) {
        EffectModule::kill_all(boma, *EFFECT_SUB_ATTRIBUTE_SYNC_STOP as u32, false, true);
    }
    0.into()
}
