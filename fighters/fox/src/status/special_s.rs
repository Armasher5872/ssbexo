...
unsafe extern "C" fn fox_special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if cont {
        if !StatusModule::is_changing(fighter.module_accessor) {
            if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_RUSH_FORCE_END) && !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_HIT_SHIELD_TO_END) {
...
unsafe extern "C" fn fox_special_s_change_mot(fighter: &mut L2CFighterCommon, motion_kind: Hash40) {
    if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE) {
        ...
        MotionModule::change_motion_inherit_frame(fighter.module_accessor, motion_kind, -1.0, 1.0, 0.0, false, false);
    }
}
...
unsafe extern "C" fn fox_special_s_exec_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let fox_illusion_step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let fox_illusion_prev_step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    ...
    if situation_kind != *SITUATION_KIND_AIR {
        WorkModule::off_flag(fighter.module_accessor, *FIGHTER_FOX_INSTANCE_WORK_ID_FLAG_ILLUSION_LANDING);
    }
    else {
        WorkModule::on_flag(fighter.module_accessor, *FIGHTER_FOX_INSTANCE_WORK_ID_FLAG_ILLUSION_LANDING);
    };
...
unsafe extern "C" fn fox_special_s_handle_step(fighter: &mut L2CFighterCommon) {
    ...
    let fox_illusion_step = WorkModule::get_int(fighter.module_accessor, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let illusion_shield_hit_end_speed_x = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_shield_hit_end_speed_x"));
    ...
    let illusion_shield_hit_end_air_speed_x = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_shield_hit_end_air_speed_x"));
    ...
    let illusion_stop_y_frame = WorkModule::get_param_int(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_stop_y_frame"));
    WorkModule::set_int(fighter.module_accessor, fox_illusion_step, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    if fox_illusion_step == *FIGHTER_FOX_ILLUSION_STEP_START {
        ...
        WorkModule::set_int(fighter.module_accessor, illusion_end_air_stop_y_frame, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
        if situation_kind != *SITUATION_KIND_AIR {
