...
unsafe extern "C" fn fun_7100018450(fighter: &mut L2CFighterCommon) {
    ...
    if situation_kind != *SITUATION_KIND_GROUND {
        ...
        WorkModule::unable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_WAIT);
        WorkModule::enable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_FALL);
        if WorkModule::is_flag(boma, *FIGHTER_FOX_REFLECTOR_STATUS_WORK_ID_FLAG_CONTINUE) {
            MotionModule::change_motion_inherit_frame(boma, Hash40::new(air_motion), -1.0, 1.0, 0.0, false, false);
        }
        else {
            MotionModule::change_motion(boma, Hash40::new(air_motion), 0.0, 1.0, false, 0.0, false, false);
            WorkModule::on_flag(boma, *FIGHTER_FOX_REFLECTOR_STATUS_WORK_ID_FLAG_CONTINUE);    
        }
    }
    else {
        ...
        WorkModule::enable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_WAIT);
        WorkModule::unable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_FALL);
        if WorkModule::is_flag(boma, *FIGHTER_FOX_REFLECTOR_STATUS_WORK_ID_FLAG_CONTINUE) {
            MotionModule::change_motion_inherit_frame(boma, Hash40::new(ground_motion), -1.0, 1.0, 0.0, false, false);
        }
        else {
            MotionModule::change_motion(boma, Hash40::new(ground_motion), 0.0, 1.0, false, 0.0, false, false);
            WorkModule::on_flag(boma, *FIGHTER_FOX_REFLECTOR_STATUS_WORK_ID_FLAG_CONTINUE);    
        }
    }
}
...
unsafe extern "C" fn fox_special_lw_end_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if MotionModule::is_end(boma) {
        if WorkModule::is_enable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_WAIT) {
            fighter.change_status(FIGHTER_STATUS_KIND_WAIT.into(), false.into());
        }
        if WorkModule::is_enable_transition_term(boma, *FIGHTER_FOX_REFLECTOR_TRANSITION_TERM_ID_FALL) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_FOX_INSTANCE_WORK_ID_FLAG_REFLECTOR_HIT) {
...
unsafe extern "C" fn fox_special_lw_end_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind != *SITUATION_KIND_AIR {
        ...
        WorkModule::on_flag(boma, *FIGHTER_FOX_INSTANCE_WORK_ID_FLAG_REFLECTOR_LANDING);
    }
