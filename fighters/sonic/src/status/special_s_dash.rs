...
unsafe extern "C" fn sonic_special_s_dash_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_SPECIAL_LW | *FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK) as u64, 0, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_LW as u32, 0);
    0.into()
}
...
unsafe extern "C" fn sonic_special_s_dash_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    WorkModule::enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_GROUP_CHK_AIR_JUMP_AERIAL);
    ...
    WorkModule::enable_transition_term_group(boma, *FIGHTER_STATUS_TRANSITION_GROUP_CHK_AIR_JUMP_AERIAL);
    if prev_status_kind != *FIGHTER_SONIC_STATUS_KIND_SPECIAL_S_TURN {
        if WorkModule::is_flag(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_FLAG_SPECIAL_LW_HOLD) {
            let special_lw_charge_level = WorkModule::get_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_WORK_INT_SPECIAL_LW_CHARGE_LEVEL);
            if special_lw_charge_level != 0 {
                if special_lw_charge_level != 1 {
                    MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new("effect_speciallwdashchargehi"), -1);
                }
                else {
                    MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new("effect_speciallwdashchargemiddle"), -1);
                }
            }
            else {
                MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new("effect_speciallwdashchargelw"), -1);
            }
        }
    }
    if situation_kind == *SITUATION_KIND_GROUND {
        GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_SONIC_SPECIAL_S_DASH);
    }
    else {
        GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_SONIC_SPECIAL_S_AIR_DASH);
    }
    MotionModule::change_motion(boma, Hash40::new_raw(0x1c90a5cf37), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn fun_71000197e0(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::dec_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_WORK_INT_ADVANCE_COUNTER);
    let special_s_dash_advance_counter = WorkModule::get_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_WORK_INT_ADVANCE_COUNTER);
...
unsafe extern "C" fn sonic_special_s_dash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let mtrans_type = WorkModule::get_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_WORK_INT_WAIT_MTRANS_TYPE);
    let special_s_dash_advance_counter = WorkModule::get_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_WORK_INT_ADVANCE_COUNTER);
    let special_s_dash_stop_speed = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_dash_stop_speed"));
    let special_s_turn_fail_speed = WorkModule::get_param_float(boma, hash40("param_special_s"), hash40("special_s_turn_fail_speed"));
    ...
    let slow_check  = if !StopModule::is_stop(boma) {SlowModule::is_skip(boma)} else {true};
    ...
    if !slow_check {
        if fun_710001aff0(fighter).get_bool() {
            if WorkModule::is_flag(boma, *FIGHTER_SONIC_STATUS_SPECIAL_S_DASH_FLAG_INFLICT_OCCUR) {
