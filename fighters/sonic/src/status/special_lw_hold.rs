...
unsafe extern "C" fn sonic_special_lw_hold_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind == *SITUATION_KIND_GROUND {
        ...
        MotionModule::change_motion(boma, Hash40::new("special_lw_hold"), 0.0, 1.0, false, 0.0, false, false);
    }
    else {
        GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_SONIC_SPECIAL_AIR_LW);
        MotionModule::change_motion(boma, Hash40::new("special_air_lw_hold"), 0.0, 1.0, false, 0.0, false, false);
    }
    MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new_raw(0x1c2f32b79c), -1);
...
unsafe extern "C" fn sonic_special_lw_hold_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let rot_speed = WorkModule::get_float(boma, *FIGHTER_SONIC_STATUS_SPECIAL_LW_HOLD_WORK_FLOAT_ROT_SPEED);
    let charge_level = WorkModule::get_int(boma, *FIGHTER_SONIC_STATUS_SPECIAL_LW_HOLD_WORK_INT_CHARGE_LEVEL);
    let squat_stick_y = WorkModule::get_param_float(boma, hash40("common"), hash40("squat_stick_y"));
    let special_lw_end_rot_speed = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("special_lw_end_rot_speed"));
    let special_lw_start_rot_speed = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("special_lw_start_rot_speed"));
    ...
    if ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL) {
        WorkModule::add_float(boma, 1.0, *FIGHTER_SONIC_STATUS_SPECIAL_LW_HOLD_WORK_FLOAT_ROT_SPEED);
    }
    if charge_level != 0 {
        if charge_level != 1 {
            if current_charge <= 0.66 {
                fun_71000156a0(fighter);
                MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new_raw(0x204be4482f), -1);
                WorkModule::set_int(boma, charge_level-1, *FIGHTER_SONIC_STATUS_SPECIAL_LW_HOLD_WORK_INT_CHARGE_LEVEL);
            }
        }
        else {
            if 0.66 >= current_charge {
                ...
                fun_71000156a0(fighter);
                MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EFFECT, Hash40::new_raw(0x1cb1514ff), -1);
                WorkModule::set_int(boma, charge_level+1, *FIGHTER_SONIC_STATUS_SPECIAL_LW_HOLD_WORK_INT_CHARGE_LEVEL);
            }
        }
    }
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && situation_kind == *SITUATION_KIND_AIR {
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_SONIC_SPECIAL_AIR_LW);
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_lw_hold"), -1.0, 1.0, 0.0, false, false);
        }
