...
unsafe extern "C" fn shield_damage_unbroken_change_status(ctx: &mut skyline::hooks::InlineCtx) {
    ...
    if attacker_id != *BATTLE_OBJECT_ID_INVALID as u32 {
        ...
        if !attacker_battle_object_vtable(attacker_battle_object) && 3 < *(attacker_battle_object as *const u8).add(0x34) {
            ...
            if FighterUtil::is_valid_just_shield(defender_boma) /*WorkModule::is_flag(defender_boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_ENABLE_PARRY)*/ && defender_status_kind == *FIGHTER_STATUS_KIND_GUARD_OFF {
                if (*shield_module_attacker_info).attacker_category == 0 {
                    if StatusModule::situation_kind(attacker_boma) == *SITUATION_KIND_AIR {
                        ...
                        KineticModule::unable_energy_all(attacker_boma);
                    }
                    else {
            ...
            if WorkModule::is_flag(attacker_boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_HARD_BREAK_ENABLED) {
                let hard_break = EffectModule::req(attacker_boma, Hash40::new("sys_flash"), &Vector3f{x: WorkModule::get_float(attacker_boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_LAST_ATTACK_HITBOX_LOCATION_X), y: WorkModule::get_float(attacker_boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_LAST_ATTACK_HITBOX_LOCATION_Y), z: 0.0}, &Vector3f::zero(), 1.5, 0, 0, false, 0) as u32;
