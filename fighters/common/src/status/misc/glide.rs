...
unsafe extern "C" fn status_glide_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::set_float(boma, params.base_speed, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_POWER);
    WorkModule::set_float(boma, -sum_speed_y, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_GRAVITY);
    let initial_speed = params.base_speed * lr;
    KineticUtility::reset_enable_energy(*FIGHTER_KINETIC_ENERGY_ID_STOP, boma, *ENERGY_STOP_RESET_TYPE_FREE, &Vector2f{x: initial_speed, y: 0.0}, &Vector3f{x: initial_speed, y: 0.0, z: 0.0});
...
unsafe extern "C" fn status_glide(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_NO_GLIDE);
    WorkModule::enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_GLIDE_LANDING);
    WorkModule::enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_GLIDE_ATTACK);
    WorkModule::enable_transition_term_group(boma, *FIGHTER_STATUS_TRANSITION_GROUP_CHK_AIR_LANDING);
    MotionModule::change_motion(boma, Hash40::new("glide_direction"), 90.0, 0.0, true, 0.0, false, false);
    MotionModule::add_motion_partial(boma, *FIGHTER_METAKNIGHT_MOTION_PART_SET_KIND_WING, Hash40::new("glide_wing"), 0.0, 1.0, true, false, 0.0, false, true, false);
...
unsafe extern "C" fn status_glide_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let sum_speed_length = KineticModule::get_sum_speed_length(boma, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    let frame = MotionModule::frame(boma);
    let glide_timer = WorkModule::get_int(boma, *FIGHTER_INSTANCE_WORK_ID_INT_GLIDE_TIMER);
    let glide_landing_frame = WorkModule::get_param_float(boma, hash40("common"), hash40("glide_landing_frame"));
    let glide_landing_speed = WorkModule::get_param_float(boma, hash40("common"), hash40("glide_landing_speed"));
    ...
    WorkModule::inc_int(boma, *FIGHTER_INSTANCE_WORK_ID_INT_GLIDE_TIMER);
    if glide_timer >= 300 {
        WorkModule::on_flag(boma, *FIGHTER_STATUS_GLIDE_FLAG_STOP);
    }
    ...
    if situation_kind == *SITUATION_KIND_AIR {
        if ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_JUMP) || ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_SPECIAL) || WorkModule::is_flag(boma, *FIGHTER_STATUS_GLIDE_FLAG_STOP) {
            fighter.change_status(FIGHTER_STATUS_KIND_GLIDE_END.into(), false.into());
            return 0.into();
        }
        if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_GLIDE_ATTACK) && ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_ATTACK) {
...
unsafe extern "C" fn status_glide_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let params = GlideParams::get();
    let lr = PostureModule::lr(boma);
    let _energy_stop = KineticModule::get_energy(boma, *FIGHTER_KINETIC_ENERGY_ID_STOP);
    let mut angle = WorkModule::get_float(boma, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_ANGLE);
    let mut angle_speed = WorkModule::get_float(boma, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_ANGLE_SPEED);
    let mut stick_angle = ControlModule::get_stick_angle(boma);
    if lr <= 0.0 {
    ...
    if stick_magnitude > params.radial_stick {
        ...
        WorkModule::set_float(boma, new_angle_speed, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_ANGLE_SPEED);
        angle += new_angle_speed;
    }
    angle = angle.clamp(params.angle_max_down, params.angle_max_up);
    let mut power = WorkModule::get_float(boma, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_POWER);
    ...
    if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_SIDE as u32) {
        power -= 0.01;
    }
    if power < 0.0 {
        power = 0.0
    }
    if !WorkModule::is_flag(boma, *FIGHTER_STATUS_GLIDE_FLAG_RAPID_FALL) {
        ...
        WorkModule::off_flag(boma, *FIGHTER_STATUS_GLIDE_FLAG_RAPID_FALL);
    }
    let gravity = WorkModule::get_float(boma, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_GRAVITY);
    let mut new_gravity = gravity + params.gravity_accel;
    if new_gravity > params.gravity_speed {
        new_gravity = params.gravity_speed;
    }
    WorkModule::set_float(boma, new_gravity, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_GRAVITY);
    ...
    if speed < params.end_speed || power <= 0.0 {
        WorkModule::on_flag(boma, *FIGHTER_STATUS_GLIDE_FLAG_STOP);
        WorkModule::set_float(boma, 0.0, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_ANGLE_SPEED);
    }
    ...
    WorkModule::set_float(boma, power, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_POWER);
    MotionModule::set_frame(boma, 90.0 - angle, false);
    WorkModule::set_float(boma, angle, *FIGHTER_STATUS_GLIDE_WORK_FLOAT_ANGLE);
    let kind = fighter.global_table[0x2].get_i32();
    if kind == *FIGHTER_KIND_METAKNIGHT {
        SoundModule::set_se_pitch_ratio(boma, Hash40::new("se_metaknight_glide_loop"), 1.0 + angle * -0.0035);
    }
    0.into()
}
