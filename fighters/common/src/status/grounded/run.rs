...
unsafe fn status_runbrake_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && item_lua_stack_throw || {fighter.clear_lua_stack(); lua_args!(fighter, MA_MSC_ITEM_CHECK_HAVE_ITEM_TRAIT, ITEM_TRAIT_FLAG_SHOOT); sv_module_access::item(fighter.lua_state_agent); 
        if fighter.pop_lua_stack(1).get_bool() {
            ItemModule::get_shoot_item_bullet(boma, 0) <= 0
        }
        else {
            false
        }
    ...
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ITEM_THROW_DASH) && pad_flag & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 && item_lua_stack_throw {
        fighter.change_status(FIGHTER_STATUS_KIND_ITEM_THROW_DASH.into(), true.into());
        return 0.into();
    }
    ...
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ITEM_SWING_DASH) && item_lua_stack_swing && pad_flag & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 {
        fighter.change_status(FIGHTER_STATUS_KIND_ITEM_SWING_DASH.into(), true.into());
        return 0.into();
    }
    ...
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ATTACK_DASH) && pad_flag & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 {
        fighter.change_status(FIGHTER_STATUS_KIND_ATTACK_DASH.into(), true.into());
        return 0.into();
    }
    ...
    || FighterUtil::is_valid_auto_catch_item(boma, false) {
        ...
        if ItemModule::get_pickable_item_size(boma) == *ITEM_SIZE_LIGHT as u64 && WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ITEM_PICKUP_LIGHT_DASH) && light_item {
    ...
    let turn_run_stick_x = WorkModule::get_param_float(boma, hash40("common"), hash40("turn_run_stick_x"));
    if WorkModule::is_flag(boma, *FIGHTER_STATUS_RUN_BRAKE_FLAG_TURN_RUN) && stick_x <= turn_run_stick_x {
        fighter.change_status(FIGHTER_STATUS_KIND_TURN_RUN.into(), false.into());
        return 0.into();
    }
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_TURN_DASH) && fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_TURN_DASH != 0 {
        fighter.change_status(FIGHTER_STATUS_KIND_TURN_DASH.into(), true.into());
        return true.into();
    }
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_DASH) && fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_DASH != 0 {
    ...
    if !WorkModule::is_flag(boma, *FIGHTER_STATUS_RUN_BRAKE_FLAG_STOP_SHAKE) {
        ...
        WorkModule::set_float(boma, speed_length, *FIGHTER_STATUS_RUN_BRAKE_WORK_FLOAT_START_SPEED);
        ...
        if speed_length < run_brake_stop_shake_speed {
            ...
            let brake_start_speed = WorkModule::get_float(boma, *FIGHTER_STATUS_RUN_BRAKE_WORK_FLOAT_START_SPEED);
            let diff = brake_start_speed - run_brake_stop_shake_speed;
            let diff2 = speed_length - run_brake_stop_shake_speed;
            let shake_data_brake_scale = WorkModule::get_param_float(boma, hash40("param_motion"), hash40("shake_data_brake_scale"));
            let run_brake_stop_shake_scale = WorkModule::get_param_float(boma, hash40("common"), hash40("run_brake_stop_shake_scale"));
            ...
            ShakeModule::set_scale_kind(boma, Hash40::new("brake"), mul);
        }
    }
    ...
    if (WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_APPEAL_U) && fighter.global_table[CMD_CAT2].get_i32() & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_HI != 0) 
    || (WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_APPEAL_LW) && fighter.global_table[CMD_CAT2].get_i32() & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_LW != 0) 
    || (WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_APPEAL_S) && (fighter.global_table[CMD_CAT2].get_i32() & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_S_L != 0 
