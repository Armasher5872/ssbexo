...
unsafe extern "C" fn ssbexo_trail_dash_grab_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 2.6, 0.0, 6.6, 4.6, Some(0.0), Some(6.6), Some(10.8), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_G);
        CATCH(agent, 1, Hash40::new("top"), 1.3, 0.0, 6.6, 3.3, Some(0.0), Some(6.6), Some(12.1), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_A);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
