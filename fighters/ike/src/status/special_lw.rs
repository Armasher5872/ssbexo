...
unsafe extern "C" fn ike_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(boma, SituationKind(*SITUATION_KIND_NONE), *FIGHTER_KINETIC_TYPE_UNIQ, *GROUND_CORRECT_KIND_KEEP as u32, GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, 0, 0, 0, 0);
...
unsafe extern "C" fn ike_special_lw_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_n_start_spd_x_mul = WorkModule::get_param_float(boma, hash40("param_special_n") as u64, hash40("special_n_start_spd_x_mul") as u64);
    let special_n_brake_spd_x = WorkModule::get_param_float(boma, hash40("param_special_n") as u64, hash40("special_n_brake_spd_x") as u64);
    ...
    if situation_kind != *SITUATION_KIND_AIR {
        ...
        WorkModule::set_int(boma, *SITUATION_KIND_GROUND, *FIGHTER_IKE_STATUS_SPECIAL_N_WORK_INT_SITUATION_PREV);
    }
    else {
        sv_kinetic_energy!(reset_energy, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, *ENERGY_STOP_RESET_TYPE_GROUND, x_mul, 0.0, 0.0, 0.0, 0.0);
        sv_kinetic_energy!(set_accel, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, 0.0, 0.0);
        ...
        WorkModule::set_int(boma, *SITUATION_KIND_AIR, *FIGHTER_IKE_STATUS_SPECIAL_N_WORK_INT_SITUATION_PREV);
    }
...
unsafe extern "C" fn ike_special_lw_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::off_flag(boma, *FIGHTER_IKE_STATUS_SPECIAL_N_FLAG_CONTINUE_MOT);
    WorkModule::off_flag(boma, *FIGHTER_IKE_STATUS_SPECIAL_N_FLAG_CHARGE_MAX);
...
unsafe extern "C" fn fun_7100020e80(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_IKE_STATUS_SPECIAL_N_FLAG_CONTINUE_MOT){
            ...
            WorkModule::on_flag(boma, *FIGHTER_IKE_STATUS_SPECIAL_N_FLAG_CONTINUE_MOT);
        }
    }
    else {
...
unsafe extern "C" fn ike_special_lw_exec_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let situation_kind = StatusModule::situation_kind(boma);
    let situation_prev = WorkModule::get_int(boma, *FIGHTER_IKE_STATUS_SPECIAL_N_WORK_INT_SITUATION_PREV);
