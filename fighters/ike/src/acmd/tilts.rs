...
unsafe extern "C" fn ssbexo_ike_forward_tilt_lw_acmd(agent: &mut L2CAgentBase) {
    ...
    FT_MOTION_RATE(agent, 1.0);
    if is_excute(agent) {
        ATTACK(agent, 0, 0, Hash40::new("top"), 13.5, 361, 97, 0, 30, 4.0, 0.0, 8.5, 10.0, Some(0.0), Some(3.5), Some(25.0), 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_POS, false, 1, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_cutup"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_IKE, *ATTACK_REGION_SWORD);
    }
    wait(agent.lua_state_agent, 2.0);
    if is_excute(agent) {
        AttackModule::clear_all(agent.module_accessor);
...
pub fn install() {
    ...
    .game_acmd("game_attacks3", ssbexo_ike_forward_tilt_acmd, Low)
    .game_acmd("game_attacks3hi", ssbexo_ike_forward_tilt_hi_acmd, Low)
    .game_acmd("game_attacks3lw", ssbexo_ike_forward_tilt_lw_acmd, Low)
    ...
    .game_acmd("game_attacklw3", ssbexo_ike_down_tilt_acmd, Low)
