...
unsafe extern "C" fn donkey_special_lw_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_DONKEY_INSTANCE_WORK_ID_FLAG_BARREL_UNLINK) {
        if ArticleModule::is_exist(fighter.module_accessor, *FIGHTER_DONKEY_GENERATE_ARTICLE_BARREL) {
            let barrel_boma = get_article_boma(fighter.module_accessor, *FIGHTER_DONKEY_GENERATE_ARTICLE_BARREL);
            LinkModule::remove_model_constraint(barrel_boma, true);
            StatusModule::change_status_request(barrel_boma, *WEAPON_DONKEY_BARREL_STATUS_KIND_ROLL, false);
        }
        WorkModule::off_flag(fighter.module_accessor, *FIGHTER_DONKEY_INSTANCE_WORK_ID_FLAG_BARREL_UNLINK);
    }
