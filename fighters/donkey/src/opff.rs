...
unsafe extern "C" fn donkey_on_start(fighter: &mut L2CFighterCommon) {
    let boma = fighter.module_accessor;
    ...
    fighter.global_table[STATUS_END_CONTROL].assign(&L2CValue::Ptr(donkey_end_control as *const () as _));
}

pub fn install() {
    Agent::new("donkey")
    .set_costume([0, 1, 2, 3, 4, 5, 6, 7].to_vec())
    .on_start(donkey_on_start)
    .install()
    ;
}