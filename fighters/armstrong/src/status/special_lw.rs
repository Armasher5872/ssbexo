...
unsafe extern "C" fn armstrong_special_lw_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind == *SITUATION_KIND_GROUND {
        armstrong_charge_move(fighter, 5.0, 14.0, 0.045, 9.0, ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL), true, "bust");
    }
    else {
        armstrong_charge_move(fighter, 5.0, 14.0, 0.045, 0.0, ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL), false, "bust");
    }
...
unsafe extern "C" fn armstrong_special_lw_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if is_damaged(boma) {
        if WorkModule::is_flag(boma, *FIGHTER_ARMSTRONG_INSTANCE_WORK_ID_FLAG_COUNTER_ACTIVE) {
