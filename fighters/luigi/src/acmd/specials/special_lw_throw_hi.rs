...
//Grounded Down Special Up Throw ACMD
unsafe extern "C" fn ssbexo_luigi_grounded_down_special_throw_hi_acmd(agent: &mut L2CAgentBase) {
...
//Aerial Down Special Up Throw ACMD
unsafe extern "C" fn ssbexo_luigi_aerial_down_special_throw_hi_acmd(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 10.0, 87, 85, 0, 55, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
    ...
    if is_excute(agent) {
        WorkModule::on_flag(boma, *FIGHTER_LUIGI_INSTANCE_WORK_ID_FLAG_SPECIAL_LW_THROW);
        ADD_SPEED_NO_LIMIT(agent, 0.0, -2.0);
    }
...
//Grounded Down Special Up Throw Effect
unsafe extern "C" fn ssbexo_luigi_grounded_down_special_throw_hi_effect(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 21.0);
    if is_excute(agent) {
...
//Aerial Down Special Up Throw Effect
unsafe extern "C" fn ssbexo_luigi_aerial_down_special_throw_hi_effect(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 17.0);
    if is_excute(agent) {
        agent.clear_lua_stack();
...
//Grounded Down Special Up Throw Sound
unsafe extern "C" fn ssbexo_luigi_grounded_down_special_throw_hi_sound(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 1.0);
    if is_excute(agent) {
...
//Aerial Down Special Up Throw Sound
unsafe extern "C" fn ssbexo_luigi_aerial_down_special_throw_hi_sound(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 1.0);
    if is_excute(agent) {
        PLAY_SE(agent, Hash40::new("se_luigi_final04"));
...
//Grounded Down Special Up Throw Expression
unsafe extern "C" fn ssbexo_luigi_grounded_down_special_throw_hi_expression(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
...
//Aerial Down Special Up Throw Expression
unsafe extern "C" fn ssbexo_luigi_aerial_down_special_throw_hi_expression(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
        slope!(agent, *MA_MSC_CMD_SLOPE_SLOPE, *SLOPE_STATUS_LR);
