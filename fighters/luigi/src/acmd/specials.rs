...
unsafe extern "C" fn ssbexo_luigi_aerial_up_special_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        WorkModule::on_flag(agent.module_accessor, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_FLAG_REVERSE_LR);
        ATTACK(agent, 0, 0, Hash40::new("top"), 18.0, 90, 80, 0, 40, 2.7, 0.0, 6.0, 5.5, None, None, None, 0.5, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_POS, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_elec"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_BAT, *ATTACK_REGION_PUNCH);
        WorkModule::on_flag(agent.module_accessor, *FIGHTER_LUIGI_STATUS_SPECIAL_HI_FLAG_CRITICAL_HIT);
    }
    wait(agent.lua_state_agent, 1.0);
    if is_excute(agent) {
...
unsafe extern "C" fn ssbexo_luigi_down_special_loop_effect(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        EFFECT_FOLLOW(agent, Hash40::new("luigi_final_vacuum"), Hash40::new("top"), 0, 6, 14, 0, 0, 0, 1, true);
    }
}
...
unsafe extern "C" fn ssbexo_luigi_down_special_loop_sound(agent: &mut L2CAgentBase) {
    if is_excute(agent) {
        PLAY_STATUS(agent, Hash40::new("se_luigi_final02"));
    }
}
    ...
    if discharge_chance > 1 {
        if is_excute(agent) {
            if WorkModule::get_param_int(agent.module_accessor, hash40("param_motion"), hash40("flip")) != 0 {
                EFFECT_FOLLOW_FLIP(agent, Hash40::new("sys_flash"), Hash40::new("sys_flash"), Hash40::new("top"), -5, offset, 2, 0, 0, 0, 1.0, true, *EF_FLIP_YZ);
            }
            else {
                EFFECT_FOLLOW(agent, Hash40::new("sys_flash"), Hash40::new("top"), -5.0 * lr, offset, 2, 0, 0, 0, 1.0, true);
            }
            if discharge_chance == 9 {
                LAST_EFFECT_SET_COLOR(agent, 1.0, 1.0, 1.0); //White
            }
            else if discharge_chance == 7 {
                LAST_EFFECT_SET_COLOR(agent, 1.0, 0.0, 0.0); //Red
            }
            else if discharge_chance == 5 {
                LAST_EFFECT_SET_COLOR(agent, 1.0, 0.647, 0.0); //Orange
            }
            else {
                LAST_EFFECT_SET_COLOR(agent, 0.831, 0.686, 0.216); //Yellow
            }
        }
    }
    else {
        if is_excute(agent) {
            ...
            LAST_EFFECT_SET_COLOR(agent, 0.0, 1.0, 0.0); //Green
        }
    }
}
...
pub fn install() {
    ...
    .game_acmd("game_specials", ssbexo_luigi_side_special_acmd, Low)
    .game_acmd("game_specialsdischarge", ssbexo_luigi_side_special_misfire_acmd, Low)
