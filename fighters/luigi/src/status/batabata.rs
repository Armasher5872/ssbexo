...
unsafe extern "C" fn luigi_batabata_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, 0, (*FIGHTER_STATUS_ATTR_INTO_DOOR | *FIGHTER_STATUS_ATTR_START_TURN) as u32, *FIGHTER_POWER_UP_ATTACK_BIT_ATTACK_AIR as u32, 0);
    0.into()
}
...
unsafe extern "C" fn luigi_batabata_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let air_speed = WorkModule::get_param_float(boma, hash40("air_speed_x_stable"), 0);
...
unsafe extern "C" fn luigi_batabata_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::on_flag(boma, *FIGHTER_LUIGI_INSTANCE_WORK_ID_FLAG_BATABATA);
    MotionModule::change_motion(boma, Hash40::new("bata_bata"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn luigi_batabata_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ControlModule::check_button_off(boma, *CONTROL_PAD_BUTTON_JUMP) && frame < 105.0 {
        MotionModule::set_frame(boma, 106.0, false);
    }
