...
unsafe extern "C" fn luigi_special_lw_plunger_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    ArticleModule::change_status(boma, *FIGHTER_LUIGI_GENERATE_ARTICLE_OBAKYUMU, *WEAPON_LUIGI_OBAKYUMU_STATUS_KIND_SPECIAL_LW_PLUNGER, ArticleOperationTarget(0));
...
unsafe extern "C" fn luigi_special_lw_plunger_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_AIR {
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND_CLIFF_STOP_ATTACK));
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_GROUND_STOP);
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_lw_plunger"), -1.0, 1.0, 0.0, false, false);
        }
        if situation_kind == *SITUATION_KIND_AIR
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_lw_plunger"), -1.0, 1.0, 0.0, false, false);
        }
    }
    if ArticleModule::is_exist(boma, *FIGHTER_LUIGI_GENERATE_ARTICLE_PLUNGER) {
        ...
        if plunger_status_kind == *WEAPON_LUIGI_PLUNGER_STATUS_KIND_SHOOT {
            if MotionModule::frame(plunger_boma) > 1.0 {
                ModelModule::set_joint_translate(boma, Hash40::new("throw"), &Vector3f{x: collision_pos1_x, y: collision_pos1_y, z: collision_pos1_z}, true, false);
            }
        }
    }
