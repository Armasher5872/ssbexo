...
unsafe extern "C" fn luigi_obakyumu_special_lw_catch_pull_main_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(owner_boma, *FIGHTER_LUIGI_INSTANCE_WORK_ID_FLAG_SPECIAL_LW_PLUNGER_THROW) {
        MotionModule::change_motion(boma, Hash40::new("special_lw_catch_pull_plunger"), 0.0, 1.0, false, 0.0, false, false);
    }
    else {
        if owner_lr == -1.0 {
            MotionModule::change_motion(boma, Hash40::new("special_lw_catch_pull_l"), 0.0, 1.0, false, 0.0, false, false);
            MotionModule::set_flip(boma, false, true, true);
        }
        else {
            MotionModule::change_motion(boma, Hash40::new("special_lw_catch_pull"), 0.0, 1.0, false, 0.0, false, false);
        }
    }
