...
unsafe extern "C" fn luigi_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ArticleModule::is_exist(fighter.module_accessor, *FIGHTER_LUIGI_GENERATE_ARTICLE_OBAKYUMU) && object_id != *BATTLE_OBJECT_ID_INVALID {
        StatusModule::set_status_kind_interrupt(fighter.module_accessor, *FIGHTER_LUIGI_STATUS_KIND_SPECIAL_LW_START);
        1.into()
    } 
    else {
