...
unsafe extern "C" fn luigi_check_jump_uniq(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let allow_float = ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_JUMP) && !is_aerial && stick_y <= squat_stick_y;
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_AERIAL_BUTTON) 
    || WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_AERIAL) {
        if !WorkModule::is_flag(boma, *FIGHTER_LUIGI_INSTANCE_WORK_ID_FLAG_BATABATA) {
            if allow_float {
