...
unsafe extern "C" fn link_mortal_draw_loop_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("mortal_draw_loop"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn link_mortal_draw_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ControlModule::check_button_off(boma, *CONTROL_PAD_BUTTON_APPEAL_HI) {
        MotionModule::change_motion(boma, Hash40::new("appeal_hi_r"), 95.0, 1.0, false, 5.0, true, false);
    }
