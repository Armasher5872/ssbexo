...
unsafe extern "C" fn link_boomerang_turn_end_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if ![*WN_LINK_BOOMERANG_STATUS_KIND_FLY, *WN_LINK_BOOMERANG_STATUS_KIND_HOP, *WN_LINK_BOOMERANG_STATUS_KIND_TURN, *WN_LINK_BOOMERANG_STATUS_KIND_SWALLOWED].contains(&status_kind) {
        if fuse_item_id != *BATTLE_OBJECT_ID_INVALID as u32 && sv_battle_object::is_active(fuse_item_id) && sv_battle_object::category(fuse_item_id) == *BATTLE_OBJECT_CATEGORY_ITEM {
...
pub fn install() {
    ...
    .status(End, *WN_LINK_BOOMERANG_STATUS_KIND_TURN, link_boomerang_turn_end_status)
    .install()
    ;
}