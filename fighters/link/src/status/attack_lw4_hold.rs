...
unsafe extern "C" fn link_attack_lw_4_hold_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(boma, SituationKind(*SITUATION_KIND_GROUND), *FIGHTER_KINETIC_TYPE_MOTION, *GROUND_CORRECT_KIND_GROUND_CLIFF_STOP as u32, GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_ATTACK_4_HOLD_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_ATTACK_4_HOLD_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_ATTACK_4_HOLD_FLOAT, (*FS_SUCCEEDS_KEEP_EFFECT | *FS_SUCCEEDS_KEEP_VISIBILITY | *FS_SUCCEEDS_KEEP_SOUND | *FS_SUCCEEDS_KEEP_NO_REACTION) as i32);
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, log_mask_flag as u64, 0, 0, 0);
    0.into()
}
...
unsafe extern "C" fn link_attack_lw_4_hold_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let attack_lw4_hold_frame = WorkModule::get_param_int(boma, hash40("attack_lw4_hold_frame"), 0);
    let attack_lw4_hold_keep_frame = WorkModule::get_param_int(boma, hash40("attack_lw4_hold_keep_frame"), 0);
    ...
    WorkModule::set_int(boma, attack_lw4_hold_keep_frame, *FIGHTER_STATUS_ATTACK_WORK_INT_SMASH_HOLD_KEEP_FRAME);
    if stick_x >= 0.5 {
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION_LOOP);
        GroundModule::set_correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
        MotionModule::change_motion(boma, Hash40::new("attack_lw4_hold_walk_f"), 0.0, attack_lw4_hold_keep_frame as f32/ratio, false, 0.0, false, false);
    }
    else if stick_x <= -0.5 {
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION_LOOP);
        GroundModule::set_correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
        MotionModule::change_motion(boma, Hash40::new("attack_lw4_hold_walk_b"), 0.0, attack_lw4_hold_keep_frame as f32/ratio, false, 0.0, false, false);
    }
    else {
        MotionModule::change_motion(boma, Hash40::new("attack_lw4_hold"), 0.0, attack_lw4_hold_keep_frame as f32/ratio, false, 0.0, false, false);
    }
...
unsafe extern "C" fn link_attack_lw_4_hold_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind != *SITUATION_KIND_AIR {
        if 0 < mini_jump_attack_frame {
            if !StopModule::is_stop(boma) {
                if fighter.sub_check_button_jump().get_bool() {
                    MotionAnimcmdModule::call_script_single(boma, *FIGHTER_ANIMCMD_EXPRESSION, Hash40::new_raw(0x10f40d7b92), -1);
        ...
        if current_frame >= 59.0 {
            WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FULL_SMASH_ATTACK);
        }
        if !WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FULL_SMASH_ATTACK) && motion_kind == hash40("attack_lw4_hold") {
            if stick_x >= 0.5 {
                KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION_LOOP);
                GroundModule::set_correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
                MotionModule::change_motion_inherit_frame_keep_rate(boma, Hash40::new("attack_lw4_hold_walk_f"), -1.0, 1.0, 0.0);
            }
            if stick_x <= -0.5 {
                KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION_LOOP);
                GroundModule::set_correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
                MotionModule::change_motion_inherit_frame_keep_rate(boma, Hash40::new("attack_lw4_hold_walk_b"), -1.0, 1.0, 0.0);
            }
        }
        if [hash40("attack_lw4_hold_walk_f"), hash40("attack_lw4_hold_walk_b")].contains(&motion_kind) {
            ...
            if (-0.5..0.5).contains(&stick_x) {
                MotionModule::change_motion_inherit_frame_keep_rate(boma, Hash40::new("attack_lw4_hold"), -1.0, 1.0, 0.0);
            }
...
unsafe extern "C" fn link_attack_lw_4_hold_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FULL_SMASH_ATTACK) {
        WorkModule::on_flag(boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLAG_URBOSA_FURY);
    }
    0.into()
}
