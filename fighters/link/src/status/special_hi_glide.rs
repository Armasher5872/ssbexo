...
unsafe extern "C" fn link_special_hi_glide_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, (*FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_SPECIAL_HI | *FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK) as u64, *FIGHTER_STATUS_ATTR_INTO_DOOR as u32, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_HI as u32, 0);
    0.into()
}
...
unsafe extern "C" fn link_special_hi_glide_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("special_hi_glide"), 0.0, 1.0, false, 0.0, false, false);
    if !ArticleModule::is_exist(boma, *FIGHTER_LINK_GENERATE_ARTICLE_PARASAIL) {
        ArticleModule::generate_article(boma, *FIGHTER_LINK_GENERATE_ARTICLE_PARASAIL, true, -1);
    }
    ArticleModule::change_motion(boma, *FIGHTER_LINK_GENERATE_ARTICLE_PARASAIL, Hash40::new("glide"), false, -1.0);
...
unsafe extern "C" fn link_special_hi_glide_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let get_stick_prev_y = ControlModule::get_stick_prev_y(boma);
    let lr = PostureModule::lr(boma);
    let slow_rate = SlowModule::rate(boma);
    let special_hi_degree = WorkModule::get_float(boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLOAT_SPECIAL_HI_DEGREE);
    ...
    if stick_x*lr > 0.25 {
        if special_hi_degree < max_degree {
            WorkModule::set_float(boma, special_hi_degree+change_degree_per_frame, *FIGHTER_LINK_INSTANCE_WORK_ID_FLOAT_SPECIAL_HI_DEGREE);
        }
        if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_LEFT as u32) || GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_RIGHT as u32) {
            if GroundModule::is_attachable(boma, GroundTouchFlag(*GROUND_TOUCH_FLAG_LEFT)) || GroundModule::is_attachable(boma, GroundTouchFlag(*GROUND_TOUCH_FLAG_RIGHT)) {
    ...
    change_angle(boma, special_hi_degree, max_degree, "special_hi_glide_f", "special_hi_glide_b");
    if ArticleModule::is_exist(boma, *FIGHTER_LINK_GENERATE_ARTICLE_PARASAIL) {
        let parasail_boma = get_article_boma(boma, *FIGHTER_LINK_GENERATE_ARTICLE_PARASAIL);
