...
unsafe extern "C" fn link_bowarrow_fly_init_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    WorkModule::set_float(boma, WorkModule::get_float(owner_boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLOAT_SPECIAL_N_DEGREE), *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_SHOT_ANGLE);
    ...
    let power_min = match arrow_type {
        ...
        _ => {
            WorkModule::get_param_float(boma, hash40("param_bowarrow"), hash40("power_min"))
        }
    };
    let power_max = match arrow_type {
        ...
        _ => {
            WorkModule::get_param_float(boma, hash40("param_bowarrow"), hash40("power_max"))
        }
    };
    let speed_min = match arrow_type {
        ...
        _ => {
            WorkModule::get_param_float(boma, hash40("param_bowarrow"), hash40("speed_min"))
        }
    };
    let speed_max = match arrow_type {
        ...
        _ => {
            WorkModule::get_param_float(boma, hash40("param_bowarrow"), hash40("speed_max"))
        }
    };
    let accel_y = match arrow_type {
        ...
        _ => {
            WorkModule::get_param_float(boma, hash40("param_bowarrow"), hash40("accel_y"))
        }
    };
    let charge = WorkModule::get_float(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_CHARGE);
    let shot_angle = WorkModule::get_float(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_SHOT_ANGLE);
    ...
    if WorkModule::is_flag(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLAG_DOUBLE) {
        WorkModule::set_int(boma, ((power_min+power_charge)*0.75) as i32, *WN_LINK_BOWARROW_STATUS_FLY_WORK_INT_ATTACK_POWER);
    }
    else {
        WorkModule::set_int(boma, (power_min+power_charge) as i32, *WN_LINK_BOWARROW_STATUS_FLY_WORK_INT_ATTACK_POWER);
    }
    if arrow_type == *WN_LINK_BOWARROW_NORMAL_ARROW {
        PLAY_SE(weapon, Hash40::new("se_link_special_n03"));
    }
    if arrow_type == *WN_LINK_BOWARROW_FIRE_ARROW {
        PLAY_SE(weapon, Hash40::new("se_link_special_n06"));
    }
    if arrow_type == *WN_LINK_BOWARROW_SHOCK_ARROW {
        PLAY_SE(weapon, Hash40::new("se_link_special_n07"));
    }
    if arrow_type == *WN_LINK_BOWARROW_ICE_ARROW {
        PLAY_SE(weapon, Hash40::new("se_link_special_n08"));
    }
    if arrow_type == *WN_LINK_BOWARROW_LIGHT_ARROW {
        if WorkModule::is_flag(owner_boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLAG_SPECIAL_N_MAX_CHARGE) {
            PLAY_SE(weapon, Hash40::new("se_link_special_n10"));
            EFFECT_FOLLOW(weapon, Hash40::new("link_light_arrow_aura"), Hash40::new("arrow"), 0, 0, 13, -shot_angle, 0, 0, 1.0, true);
            EFFECT(weapon, Hash40::new("link_light_arrow"), Hash40::new("top"), (0.0-(0.105*shot_angle.abs()))*lr, -0.5+(0.09*shot_angle), 0, -shot_angle, 0, 0, 1, 0, 0, 0, 0, 0, 0, true);
            ModelModule::set_scale(boma, 0.001);
        }
    }
    PostureModule::set_pos(boma, &Vector3f{x: owner_pos_x+x_pos, y: owner_pos_y+y_pos, z: owner_pos_z});
    sv_kinetic_energy!(set_speed, weapon, *WEAPON_KINETIC_ENERGY_RESERVE_ID_NORMAL, speed_x+x_speed_mod, speed_y);
...
unsafe extern "C" fn link_bowarrow_fly_main_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let boma = weapon.module_accessor;
    let attack_power = WorkModule::get_int(boma, *WN_LINK_BOWARROW_STATUS_FLY_WORK_INT_ATTACK_POWER);
    MotionModule::change_motion(boma, Hash40::new("fly"), 0.0, 1.0, false, 0.0, false, false);
    AttackModule::set_power(boma, 0, attack_power as f32, false);
...
unsafe extern "C" fn link_bowarrow_fly_sub_status(weapon: &mut L2CWeaponCommon, bool_check: L2CValue) -> L2CValue {
    ...
    let rotate_z = WorkModule::get_float(boma, *WN_LINK_BOWARROW_STATUS_FLY_WORK_FLOAT_ROTATE_Z);
    let rotate_speed = WorkModule::get_float(boma, *WN_LINK_BOWARROW_STATUS_FLY_WORK_FLOAT_ROTATE_SPEED);
    ...
    WorkModule::set_float(boma, z, *WN_LINK_BOWARROW_STATUS_FLY_WORK_FLOAT_ROTATE_Z);
    0.into()
}
...
unsafe extern "C" fn link_bowarrow_fly_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let boma = weapon.module_accessor;
    let arrow_type = WorkModule::get_int(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_INT_ARROW_TYPE);
    let ret = GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_ALL as u32);
    if ret {
        if arrow_type != *WN_LINK_BOWARROW_LIGHT_ARROW {
