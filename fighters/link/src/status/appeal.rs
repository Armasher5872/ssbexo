...
unsafe extern "C" fn link_appeal_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_Appeal_common_uniq(L2CValue::Void());
    fighter.sub_shift_status_main(L2CValue::Ptr(link_appeal_main_loop as *const () as _))
}

unsafe extern "C" fn link_appeal_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if current_frame >= 2.0 {
        ...
        taunt_hold(fighter, mot as u64, WorkModule::get_float(boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_APPEAL_RESET_FRAME));
        //Mortal Draw
        if [hash40("appeal_hi_r"), hash40("appeal_hi_l")].contains(&motion_kind) && (80.0..=95.0).contains(&current_frame) && ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_APPEAL_HI) {
            fighter.change_status(FIGHTER_LINK_STATUS_KIND_MORTAL_DRAW_LOOP.into(), false.into());
        }
        if 0 < attack_kind {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_APPEAL, link_appeal_main_status)
    .install()
    ;
}