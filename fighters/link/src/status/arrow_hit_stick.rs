...
unsafe extern "C" fn fun_710002bc00(weapon: &mut L2CWeaponCommon) {
    let boma = weapon.module_accessor;
    let stick_x = WorkModule::get_float(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_STICK_X);
    let stick_y = WorkModule::get_float(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_STICK_Y);
    let stick_z = WorkModule::get_float(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLOAT_STICK_Z);
    ModelModule::set_joint_rotate(boma, Hash40::new("root"), &Vector3f{x: stick_x, y: stick_y, z: stick_z}, MotionNodeRotateCompose{_address: *MOTION_NODE_ROTATE_COMPOSE_NONE as u8}, MotionNodeRotateOrder{_address: *MOTION_NODE_ROTATE_ORDER_XYZ as u8});
}
