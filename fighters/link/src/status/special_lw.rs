...
unsafe extern "C" fn link_special_lw_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let bomb_object_id = WorkModule::get_int(boma, *FIGHTER_LINK_INSTANCE_WORK_ID_INT_BOMB_OBJECT_ID);
    ...
    if active_item_from_id != 0 {
        ...
        if object_id == item_owner_id {
            ...
            if ArticleModule::is_exist(boma, *FIGHTER_LINK_GENERATE_ARTICLE_BOOMERANG) {
                let boomerang_boma = get_article_boma(boma, *FIGHTER_LINK_GENERATE_ARTICLE_BOOMERANG);
                ...
                if fuse_item_id != *BATTLE_OBJECT_ID_INVALID as u32 && sv_battle_object::is_active(fuse_item_id) && fuse_item_id == bomb_object_id as u32 {
                    if item_status_kind != *ITEM_STATUS_KIND_STANDBY {
                        StatusModule::set_status_kind_interrupt(boma, *FIGHTER_LINK_STATUS_KIND_SPECIAL_LW_BLAST);
                        return 1.into();
                    }
                }
                else {
        ...
        WorkModule::set_int(boma, *BATTLE_OBJECT_ID_INVALID, *FIGHTER_LINK_INSTANCE_WORK_ID_INT_BOMB_OBJECT_ID);
    }
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, *FIGHTER_LOG_MASK_FLAG_ATTACK_KIND_SPECIAL_LW as u64, *FIGHTER_STATUS_ATTR_START_TURN as u32, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_LW as u32, 0);
    0.into()
}
