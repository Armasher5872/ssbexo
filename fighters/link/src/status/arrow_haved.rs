        ...
        if bow_step == *FIGHTER_LINK_STATUS_BOW_STEP_HOLD {
            if !WorkModule::is_flag(owner_boma, *FIGHTER_LINK_INSTANCE_WORK_ID_FLAG_SPECIAL_N_INIT_FUSE) {
                if ItemModule::is_have_item(owner_boma, 0) {
                    set_arrow_fuse_params(boma, ItemModule::get_have_item_kind(owner_boma, 0), FuseKind::FUSE);
                }
                    ...
                    if kind != *ITEM_KIND_NONE {
                        set_arrow_fuse_params(boma, kind, FuseKind::REFUSE);
                    }
                }
                else {
                ...
                if WorkModule::is_flag(boma, *WN_LINK_BOWARROW_INSTANCE_WORK_ID_FLAG_ITEM_FUSED) {
                    ...
                    if !LinkModule::is_link(item_boma, *ITEM_LINK_NO_HAVE) {
                        ...
                        LinkModule::link(item_boma, *ITEM_LINK_NO_HAVE, (*(boma)).battle_object_id);
