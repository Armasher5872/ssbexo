...
unsafe extern "C" fn link_swordbeam_fly_main_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let max_life = WorkModule::get_param_int(boma, hash40("param_swordbeam"), hash40("max_life"));
    let max_power = WorkModule::get_param_float(boma, hash40("param_swordbeam"), hash40("max_power"));
    let max_scale = WorkModule::get_param_float(boma, hash40("param_swordbeam"), hash40("max_scale"));
    let max_speed_x = WorkModule::get_param_float(boma, hash40("param_swordbeam"), hash40("max_speed_x"));
    ...
    AttackModule::set_power(boma, 0, max_power, false);
    PostureModule::set_scale(boma, max_scale, false);
...
unsafe extern "C" fn link_swordbeam_fly_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if life > 0 {
        if GroundModule::is_touch(boma, *GROUND_TOUCH_FLAG_ALL as u32) {
            ...
            if WorkModule::is_flag(boma, *WEAPON_LINK_SWORDBEAM_INSTANCE_WORK_ID_FLAG_HIT) {
                weapon.change_status(WEAPON_LINK_SWORDBEAM_STATUS_KIND_HIT.into(), false.into());
            }
        }
    }
