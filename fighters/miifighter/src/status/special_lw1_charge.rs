...
unsafe extern "C" fn miifighter_special_lw1_charge_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let armor = WorkModule::get_float(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLOAT_ARMOR_CRUSHING_THUNDER_KICK_CURRENT_ARMOR);
    ...
    && prev_situation_kind == *SITUATION_KIND_AIR {
        ...
        MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_lw1_charge"), -1.0, 1.0, 0.0, false, false);
    }
    ...
    && prev_situation_kind == *SITUATION_KIND_GROUND {
        ...
        GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
        MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_lw1_charge"), -1.0, 1.0, 0.0, false, false);
    }
    if current_frame == 30.0 {
        WorkModule::set_int(boma, 1, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_INT_ARMOR_CRUSHING_THUNDER_KICK_ATTRIBUTE);
    }
    WorkModule::set_float(boma, 12.0+(12.0*current_frame/60.0), *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLOAT_ARMOR_CRUSHING_THUNDER_KICK_CURRENT_DAMAGE);
    WorkModule::set_float(boma, armor+1.5, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLOAT_ARMOR_CRUSHING_THUNDER_KICK_CURRENT_ARMOR);
    if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_ARMOR_CRUSHING_THUNDER_KICK_ACTIVE_ARMOR) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_ARMOR_CRUSHING_THUNDER_KICK_DASH_CANCEL) {
    ...
    if MotionModule::is_end(boma) {
        WorkModule::set_int(boma, 2, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_INT_ARMOR_CRUSHING_THUNDER_KICK_ATTRIBUTE);
        fighter.change_status(FIGHTER_MIIFIGHTER_STATUS_KIND_SPECIAL_LW1_ATTACK.into(), false.into());
        return 1.into();
    }
    0.into()
