...
unsafe extern "C" fn miifighter_special_lw3_throw_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::set_float(boma, 1.0, *FIGHTER_STATUS_THROW_WORK_FLOAT_MOTION_RATE);
    ...
    if situation_kind == *SITUATION_KIND_GROUND {
        if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_COUNTER_THROW_IS_LINK) {
...
unsafe extern "C" fn miifighter_special_lw3_throw_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let counter_attack_power = WorkModule::get_float(boma, *FIGHTER_MIIFIGHTER_STATUS_COUNTER_THROW_FLOAT_ATTACK_POWER);
    let lw3_attack_mul = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("lw3_attack_mul"));
    let lw3_attack_max = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("lw3_attack_max"));
    let lw3_attack_max_for_enemy = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("lw3_attack_max_for_enemy"));
    let lw3_attack_power_limit = WorkModule::get_param_float(boma, hash40("param_special_lw"), hash40("lw3_attack_power_limit"));
    ...
    if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_COUNTER_THROW_IS_LINK) {
        ...
        WorkModule::set_int(boma, 0, *FIGHTER_STATUS_THROW_WORK_INT_TARGET_HIT_GROUP);
        WorkModule::set_int(boma, 0, *FIGHTER_STATUS_THROW_WORK_INT_TARGET_HIT_NO);
        if attack_power < lw3_attack_power_limit {
            attack_power = 0.0;
        }
        if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_STATUS_COUNTER_THROW_FLAG_IS_ATTACK_ENEMY) {
        ...
        WorkModule::set_float(boma, attack_power, *FIGHTER_MIIFIGHTER_STATUS_COUNTER_THROW_FLOAT_ATTACK_POWER);
...
unsafe extern "C" fn miifighter_special_lw3_throw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let attack_power = WorkModule::get_float(boma, *FIGHTER_MIIFIGHTER_STATUS_COUNTER_THROW_FLOAT_ATTACK_POWER);
    ...
    if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_STATUS_COUNTER_THROW_FLAG_THROW_AFTER_LANDING) {
        if prev_situation_kind != *SITUATION_KIND_GROUND {
            if situation_kind == *SITUATION_KIND_GROUND {
                GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND_CLIFF_STOP));
                if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_COUNTER_THROW_IS_LINK) {
                    KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION);
                    MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_lw3_throw_toss"), -1.0, 1.0, 0.0, false, false);
                }
                else {
                    KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_GROUND_STOP);
                    MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_lw3_throw"), -1.0, 1.0, 0.0, false, false);
                }
            }
        }
    }
    if WorkModule::is_flag(boma, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_FLAG_COUNTER_THROW_IS_LINK) {
        ...
        if 0.0 < attack_power {
            AttackModule::set_power(boma, 0, attack_power, true);
        }
        if counter_throw_object_id != *BATTLE_OBJECT_ID_INVALID {
            if sv_battle_object::category(counter_throw_object_id as u32) == *BATTLE_OBJECT_CATEGORY_WEAPON {
                let counter_throw_boma = sv_battle_object::module_accessor(counter_throw_object_id as u32);
...
unsafe extern "C" fn miifighter_special_lw3_throw_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if counter_throw_object_id != *BATTLE_OBJECT_ID_INVALID {
        ...
        WorkModule::set_int(boma, *BATTLE_OBJECT_ID_INVALID, *FIGHTER_MIIFIGHTER_INSTANCE_WORK_ID_INT_COUNTER_THROW_OBJECT_ID);
    }
    0.into()
}
