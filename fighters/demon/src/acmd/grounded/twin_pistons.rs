...
//Twin Piston 1 ACMD
unsafe extern "C" fn ssbexo_demon_twin_piston_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        WorkModule::on_flag(boma, *FIGHTER_DEMON_STATUS_ATTACK_HI_3_FLAG_INC_STEP);
    }
...
//Twin Piston 1 Effect
unsafe extern "C" fn ssbexo_demon_twin_piston_effect(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 6.0);
    if is_excute(agent) {
...
//Twin Piston 1 Sound
unsafe extern "C" fn ssbexo_demon_twin_piston_sound(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 8.0);
    if is_excute(agent) {
...
//Twin Piston 1 Expression
unsafe extern "C" fn ssbexo_demon_twin_piston_expression(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attacksquat4", ssbexo_demon_twin_piston_acmd, Low)
    .acmd("effect_attacksquat4", ssbexo_demon_twin_piston_effect, Low)
    .acmd("sound_attacksquat4", ssbexo_demon_twin_piston_sound, Low)
    .acmd("expression_attacksquat4", ssbexo_demon_twin_piston_expression, Low)
    .install()
    ;
}