...
//Demon God Fist Effect
unsafe extern "C" fn ssbexo_demon_demon_god_fist_effect(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    frame(lua_state, 5.0);
    if is_excute(agent) {
...
//Demon God Fist Sound
unsafe extern "C" fn ssbexo_demon_demon_god_fist_sound(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    frame(lua_state, 12.0);
    if is_excute(agent) {
...
//Demon God Fist Expression
unsafe extern "C" fn ssbexo_demon_demon_god_fist_expression(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    let boma = agent.module_accessor;
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attackstand7", ssbexo_demon_demon_god_fist_acmd, Low)
    .acmd("effect_attackstand7", ssbexo_demon_demon_god_fist_effect, Low)
    .acmd("sound_attackstand7", ssbexo_demon_demon_god_fist_sound, Low)
    .acmd("expression_attackstand7", ssbexo_demon_demon_god_fist_expression, Low)
    .install()
    ;
}