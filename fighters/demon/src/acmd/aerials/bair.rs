...
//Bair ACMD
unsafe extern "C" fn ssbexo_demon_bair_acmd(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 9.0);
    if is_excute(agent) {
        WorkModule::on_flag(boma, *FIGHTER_DEMON_INSTANCE_WORK_ID_FLAG_SOBAT_TURN);
    }
...
pub fn install() {
    ...
    .acmd("game_attackairb", ssbexo_demon_bair_acmd, Low)
    .install()
    ;
}