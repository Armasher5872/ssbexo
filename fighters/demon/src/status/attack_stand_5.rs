...
unsafe extern "C" fn demon_attack_stand_5_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    if !fun_710002aed0(fighter).get_bool() {
        demon_attack_stand_5_change_motion(fighter, 0.into());
        ...
        fighter.sub_shift_status_main(L2CValue::Ptr(demon_attack_stand_5_main_loop as *const () as _))
    }
    else {
        return 0.into();
    }
...
unsafe extern "C" fn demon_attack_stand_5_change_motion(fighter: &mut L2CFighterCommon, combo_index: L2CValue) {
    let boma = fighter.module_accessor;
    let motion_kind;
    ...
    if combo_index.get_i32() != 0 {
        if combo_index.get_i32() != 1 {
            motion_kind = Hash40::new("attack_stand_52");
        }
        else {
            motion_kind = Hash40::new("attack_stand_51");
        }
    }
    else {
        motion_kind = Hash40::new("attack_stand_5");
    }
        ...
        if WorkModule::is_flag(boma, *FIGHTER_DEMON_STATUS_ATTACK_COMBO_FLAG_FLASH_PUNCH) {
            ...
            if combo_count == 0 {
                if frame % 2.0 == 0.0 {
                    ...
                    demon_attack_stand_5_change_motion(fighter, 1.into());
                }
            }
        }
    }
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_DEMON_STATUS_KIND_ATTACK_STAND_5, demon_attack_stand_5_main_status)
    .install()
    ;
}