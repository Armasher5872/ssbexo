...
unsafe extern "C" fn demon_attack_stand_7_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
...
unsafe extern "C" fn demon_attack_stand_7_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_DEMON_INSTANCE_WORK_ID_FLAG_ENABLE_DEMON_GOD_FIST_TURN) {
        if stick_x*lr < -0.7 {
