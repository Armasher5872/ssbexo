...
unsafe extern "C" fn ssbexo_ganon_volley_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        let scale = WorkModule::get_float(boma, *WEAPON_GANON_VOLLEY_INSTANCE_WORK_ID_FLOAT_DAMAGE_SCALE);
...
pub fn install() {
    ...
    .acmd("game_fly", ssbexo_ganon_volley_acmd, Low)
    .acmd("effect_fly", ssbexo_ganon_volley_effect, Low)
    .acmd("sound_fly", ssbexo_ganon_volley_sound, Low)
    .install()
    ;
}