...
unsafe extern "C" fn ganon_special_n_volley_fire_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    if WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_N_CHARGED) {
...
unsafe extern "C" fn ganon_special_n_volley_fire_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let is_charged = WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_N_CHARGED);
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_AIR {
            ...
            if is_charged {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_n_volley_fire_max"), -1.0, 1.0, 0.0, false, false);
            }
            else {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_n_volley_fire"), -1.0, 1.0, 0.0, false, false);
            }
        }
        if situation_kind == *SITUATION_KIND_AIR
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            ...
            if is_charged {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_n_volley_fire_max"), -1.0, 1.0, 0.0, false, false);
            }
            else {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_n_volley_fire"), -1.0, 1.0, 0.0, false, false);
            }
        }
    }
...
unsafe extern "C" fn ganon_special_n_volley_fire_exec_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if current_frame < 11.0 {
        if !WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_N_CHARGED) {
            if stick_y >= 0.7 {
                WorkModule::set_int(boma, 1, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_N_DIRECTION);
            }
            else if stick_y <= -0.7 {
                WorkModule::set_int(boma, 2, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_N_DIRECTION);
            }
            else {
                WorkModule::set_int(boma, 0, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_N_DIRECTION);
            }
        }
    }
    0.into()
...
unsafe extern "C" fn ganon_special_n_volley_fire_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::on_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_USED_SPECIAL_N_AIR);
    0.into()
}
