...
unsafe extern "C" fn ganon_special_hi_move_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_HI_CHARGED) {
        ...
        WorkModule::set_int(boma, 30, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_HI_MOVE_FRAME);
    }
    else {
        ...
        EffectModule::set_rate(boma, effect as u32, 0.6);
        EffectModule::set_rgb(boma, effect as u32, 1.0, 0.0, 0.325);
        WorkModule::set_int(boma, effect as i32, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_EFFECT_HANDLE);
        WorkModule::set_int(boma, 7, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_HI_MOVE_FRAME);
    }
    ...
    MotionModule::change_motion(boma, Hash40::new("special_hi_move"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn ganon_special_hi_move_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let special_hi_move_frame = WorkModule::get_int(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_HI_MOVE_FRAME);
    if special_hi_move_frame <= 0 {
        fighter.change_status(FIGHTER_GANON_STATUS_KIND_SPECIAL_HI_END.into(), false.into());
        return 1.into();
    }
    if situation_kind == *SITUATION_KIND_GROUND && WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_HI_CHARGED) {
...
unsafe extern "C" fn ganon_special_hi_move_exec_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_hi_move_frame = WorkModule::get_int(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_HI_MOVE_FRAME);
    if special_hi_move_frame > 0 {
        WorkModule::dec_int(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_INT_SPECIAL_HI_MOVE_FRAME);
    }
    0.into()
}
