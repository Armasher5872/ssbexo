...
unsafe extern "C" fn ganon_appeal_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_Appeal_common_uniq(L2CValue::Void());
    fighter.sub_shift_status_main(L2CValue::Ptr(ganon_appeal_main_loop as *const () as _))
}

unsafe extern "C" fn ganon_appeal_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if current_frame >= 2.0 {
        ...
        //Triforce Banishment
        if [hash40("appeal_s_r"), hash40("appeal_s_l")].contains(&motion_kind) && (17.0..=85.0).contains(&current_frame) && ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_SPECIAL) {
            fighter.change_status(FIGHTER_GANON_STATUS_KIND_APPEAL_ATTACK.into(), false.into());
        }
        if 0 < attack_kind {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_APPEAL, ganon_appeal_main_status)
