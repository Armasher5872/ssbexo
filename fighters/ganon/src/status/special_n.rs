...
unsafe extern "C" fn ganon_special_n_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind == *SITUATION_KIND_AIR {
        ...
        if !WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_USED_SPECIAL_N_AIR) {
            sv_kinetic_energy!(reset_energy, fighter, FIGHTER_KINETIC_ENERGY_ID_GRAVITY, ENERGY_GRAVITY_RESET_TYPE_GRAVITY, 0.0, get_sum_speed_y*0.08, 0.0, 0.0, 0.0);
...
unsafe extern "C" fn ganon_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_SPECIAL_N_TRANSITION_ENABLE) {
        if ControlModule::check_button_on_trriger(boma, *CONTROL_PAD_BUTTON_SPECIAL) {
            if !WorkModule::is_flag(boma, *FIGHTER_GANON_INSTANCE_WORK_ID_FLAG_HAS_ACTIVE_VOLLEY) {
