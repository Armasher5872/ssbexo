...
unsafe extern "C" fn cloud_special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            ...
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_FALL);
            if is_punisher {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("punish_special_air_s"), -1.0, 1.0, 0.0, false, false);
            }
            else {
...
unsafe extern "C" fn cloud_special_s_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let situation_kind = fighter.global_table[SITUATION_KIND].get_i32();
    let boma = fighter.module_accessor;
    if situation_kind == *SITUATION_KIND_AIR && WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
