...
unsafe extern "C" fn cloud_status_jump_sub(fighter: &mut L2CFighterCommon, motion_kind: L2CValue, value: L2CValue) -> L2CValue {
    ...
    let mini_jump = WorkModule::is_flag(boma, *FIGHTER_STATUS_WORK_ID_FLAG_RESERVE_JUMP_MINI);
    ...
    if !WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_POWBLOCK_QUAKE_JUMP) {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_RABBIT_CAP) {
            SoundModule::play_se(boma, Hash40::new("se_item_usagihat_jump_01"), true, false, false, false, enSEType(0));
        }
    }
    else {
        if is_punisher {
    ...
    MotionModule::change_motion(boma, Hash40::new_raw(ret), 0.0, 1.0, false, value.get_f32(), false, false);
