...
unsafe extern "C" fn cloud_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let control_limit_mul_x = WorkModule::get_param_float(boma, hash40("param_special_n"), hash40("control_limit_mul_x"));
    let limit_level = WorkModule::get_int(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_INT_LIMIT_LEVEL);
    let special_wait_timer = WorkModule::get_int(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_INT_SPECIAL_INPUT_WAIT_TIMER);
    ...
    if ControlModule::check_button_on_trriger(boma, *CONTROL_PAD_BUTTON_SPECIAL) && special_wait_timer <= 0 {
        WorkModule::set_int(boma, 15, *FIGHTER_CLOUD_INSTANCE_WORK_ID_INT_SPECIAL_INPUT_WAIT_TIMER);
    }
    if special_wait_timer > 0 {
        WorkModule::dec_int(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_INT_SPECIAL_INPUT_WAIT_TIMER);
    }
...
unsafe extern "C" fn cloud_special_n_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL) {
        ...
        WorkModule::off_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_LIMIT_BREAK_SPECIAL);
        WorkModule::off_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_LIMIT_BREAK_SET_CUSTOM);
        EffectModule::kill_kind(boma, Hash40::new("cloud_limitbreak_aura"), true, true);
        EffectModule::kill_kind(boma, Hash40::new("cloud_limitbreak"), true, true);
        EffectModule::kill_kind(boma, Hash40::new_raw(0x16102a334b), true, true);
        EffectModule::kill_kind(boma, Hash40::new_raw(0x14d013ba16), true, true);
        EffectModule::kill(boma, limit_gauge_effect as u32, true, true);
        WorkModule::set_int64(boma, 0, *FIGHTER_CLOUD_INSTANCE_WORK_ID_INT_LIMIT_GAUGE_EFFECT);
