...
unsafe extern "C" fn cloud_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(boma) {
        ...
        && prev_situation_kind == *SITUATION_KIND_GROUND {
            ...
            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_AIR_STOP);
            if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
                MotionModule::change_motion_inherit_frame(boma, Hash40::new("punish_special_air_lw"), -1.0, 1.0, 0.0, false, false);
            }
            else {
...
unsafe extern "C" fn cloud_special_lw_exit_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if status_kind != *FIGHTER_CLOUD_STATUS_KIND_SPECIAL_LW_LIMIT_BREAK {
        if !WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
            cloud_set_lightweight(boma, true);
            WorkModule::on_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE);
        }
        else {
