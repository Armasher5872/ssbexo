...
unsafe extern "C" fn cloud_sub_fall_uniq_process_init_param(fighter: &mut L2CFighterCommon, param_2: L2CValue, param_3: L2CValue, param_4: L2CValue) {
    ...
    let fall_x_accel_mul = WorkModule::get_float(boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_FALL_X_ACCEL_MUL);
    let mul_fall_x_accel = WorkModule::get_float(boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_MUL_FALL_X_ACCEL);
    let fall_x_max_mul = WorkModule::get_float(boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_FALL_X_MAX_MUL);
    let fall_hop_y = WorkModule::get_float(boma, *FIGHTER_INSTANCE_WORK_ID_FLOAT_FALL_HOP_Y);
    let entry_id = WorkModule::get_int(boma, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID);
    let final_end_fall_speed_y = WorkModule::get_param_float(boma, hash40("common"), hash40("final_end_fall_speed_y"));
    ...
    if status_kind_interrupt != param_2.get_i32() {
        if status_kind_interrupt == param_3.get_i32() {
            if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
                WorkModule::set_int64(boma, hash40("punish_fall_aerial_f") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_F);
                WorkModule::set_int64(boma, hash40("punish_fall_aerial_b") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_B);
            }
            else {
                WorkModule::set_int64(boma, hash40("fall_aerial_f") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_F);
                WorkModule::set_int64(boma, hash40("fall_aerial_b") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_B);
            }
        }
        if status_kind_interrupt == *FIGHTER_STATUS_KIND_FALL_SPECIAL {
            WorkModule::set_int64(boma, hash40("fall_special_f") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_F);
            WorkModule::set_int64(boma, hash40("fall_special_b") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_B);
            ...
            EffectModule::req_common(boma, Hash40::new("fall_special"), 0.0);
        }
    }
    else {
        if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
            WorkModule::set_int64(boma, hash40("punish_fall_f") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_F);
            WorkModule::set_int64(boma, hash40("punish_fall_b") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_B);
        }
        else {
            WorkModule::set_int64(boma, hash40("fall_f") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_F);
            WorkModule::set_int64(boma, hash40("fall_b") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_B);
        }
        if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FINAL_END_TO_FALL) {
            sv_kinetic_energy!(set_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_GRAVITY, final_end_fall_speed_y);
        }
        else {
            if WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FALL_HOP) {
                WorkModule::off_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_FALL_HOP);
                sv_kinetic_energy!(set_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_GRAVITY, fall_hop_y);
            }
        }
    }
    WorkModule::set_int64(boma, hash40("invalid") as i64, *FIGHTER_STATUS_FALL_WORK_INT_MOTION_KIND_2ND);
    WorkModule::set_float(boma, 0.0, *FIGHTER_STATUS_FALL_WORK_FLOAT_MOTION_VALUE);
    ...
    if ![param_4.get_i32(), *FIGHTER_STATUS_KIND_RUN, *FIGHTER_STATUS_KIND_DASH, *FIGHTER_STATUS_KIND_TURN_DASH].contains(&prev_status_kind) {
        ...
        WorkModule::on_flag(boma, *FIGHTER_STATUS_FALL_FLAG_FROM_GROUND);
    }
}
...
unsafe extern "C" fn cloud_status_fall_sub(fighter: &mut L2CFighterCommon, value: L2CValue) {
    let boma = fighter.module_accessor;
    let motion = if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {"punish_fall"} else {"fall"};
...
unsafe extern "C" fn cloud_fall_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let fall_frame = WorkModule::get_int(boma, *FIGHTER_STATUS_FALL_WORK_INT_FRAME);
    let jump_no_aerial_frame = WorkModule::get_param_int(boma, hash40("common"), hash40("jump_no_aerial_frame"));
    let jump_run_stick_y = WorkModule::get_param_float(boma, hash40("common"), hash40("jump_run_stick_y"));
    if !fighter.sub_transition_group_check_air_cliff().get_bool() {
        if !fighter.sub_fall().get_bool() {
            if fall_frame <= jump_no_aerial_frame {
                if !ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_CSTICK_ON) {
                    if fighter.sub_check_button_jump().get_bool() {
                        if WorkModule::is_flag(boma, *FIGHTER_STATUS_FALL_FLAG_FROM_GROUND) {
                            fighter.change_status(FIGHTER_STATUS_KIND_JUMP.into(), true.into());
                            return 0.into();
                        }
                    }
                }
                if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_SQUAT) {
            ...
            if !fighter.sub_air_check_fall_common().get_bool() {
                if WorkModule::is_flag(boma, *FIGHTER_STATUS_FALL_FLAG_DISABLE_MOTION_INTP) {
                    if MotionModule::is_end(boma) {
                        if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
                            MotionModule::change_motion(boma, Hash40::new("punish_fall"), 0.0, 1.0, false, 0.0, false, false);
                        }
                        else {
                            MotionModule::change_motion(boma, Hash40::new("fall"), 0.0, 1.0, false, 0.0, false, false);
                        }
                        WorkModule::off_flag(boma, *FIGHTER_STATUS_FALL_FLAG_DISABLE_MOTION_INTP);
                    }
                }
