...
unsafe extern "C" fn cloud_squat_wait_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
        MotionModule::change_motion_force_inherit_frame(boma, Hash40::new("punish_squat_wait"), 0.0, 1.0, 0.0);
    }
...
unsafe extern "C" fn cloud_squat_wait_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let squat_frame = WorkModule::get_int(boma, *FIGHTER_STATUS_SQUAT_WORK_INT_FRAME);
    ...
    if WorkModule::is_enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_SQUAT_RV) {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_STATUS_SQUAT_FLAG_ENABLE_PASS) {
        if squat_frame == 0 {
            if situation_kind == *SITUATION_KIND_GROUND {
    ...
    if can_loop {
        ...
        if item_function != hash40("squat_wait_item") {
            ...
            if switch_squat_motion == 1 {
                MotionModule::change_motion(boma, Hash40::new("squat_wait2"), 0.0, 1.0, false, 0.0, false, false);
            }
            if switch_squat_motion == -1 {
                MotionModule::set_frame_sync_anim_cmd(boma, 0.0, true, false, false);
            }
            if WorkModule::is_flag(boma, *FIGHTER_CLOUD_INSTANCE_WORK_ID_FLAG_PUNISHER_MODE) {
                MotionModule::change_motion(boma, Hash40::new("punish_squat_wait"), 0.0, 1.0, false, 0.0, false, false);
            }
            else {
                MotionModule::change_motion(boma, Hash40::new("squat_wait"), 0.0, 1.0, false, 0.0, false, false);
            }
        }
        else {
            MotionModule::change_motion(boma, Hash40::new("squat_wait_item"), 0.0, 1.0, false, 0.0, false, false);
        }
    }
    0.into()
}
