...
unsafe extern "C" fn cloud_guard_off_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(boma, SituationKind(*SITUATION_KIND_GROUND), *FIGHTER_KINETIC_TYPE_MOTION, *GROUND_CORRECT_KIND_GROUND_CLIFF_STOP as u32, GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_GUARD_OFF_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_GUARD_OFF_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_GUARD_OFF_FLOAT, 0);
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, true, false, false, 0, *FIGHTER_STATUS_ATTR_DISABLE_SHIELD_RECOVERY as u32, 0, 0);
    0.into()
}
...
unsafe extern "C" fn cloud_guard_off_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    MotionModule::change_motion(boma, Hash40::new("punish_guard_off"), 0.0, 1.0, false, 0.0, false, false);
    WorkModule::enable_transition_term(boma, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ITEM_THROW_GUARD);
