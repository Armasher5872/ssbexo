...
unsafe extern "C" fn ryu_wait_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.sub_wait_common();
    fighter.sub_wait_motion_mtrans();
    fighter.sub_shift_status_main(L2CValue::Ptr(ryu_wait_main_loop as *const () as _))
}

unsafe extern "C" fn ryu_wait_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    ...
    .status(Main, *FIGHTER_STATUS_KIND_WAIT, ryu_wait_main_status)
    .install()
    ;
}