...
unsafe extern "C" fn ssbexo_ryu_hadoken_move_w_acmd(agent: &mut L2CAgentBase) {
    ...
    if WorkModule::is_flag(boma, *WEAPON_RYU_HADOKEN_INSTANCE_WORK_ID_FLAG_IS_HASOGEKI) {
        if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_movew", ssbexo_ryu_hadoken_move_w_acmd, Low)
    .acmd("game_movem", ssbexo_ryu_hadoken_move_m_acmd, Low)
    .acmd("game_moves", ssbexo_ryu_hadoken_move_s_acmd, Low)
    .install()
    ;
}