...
unsafe extern "C" fn ssbexo_ryu_hadoken_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        if WorkModule::is_flag(boma, *FIGHTER_RYU_INSTANCE_WORK_ID_FLAG_IS_HASOGEKI) {
    ...
    if is_excute(agent) {
        if WorkModule::is_flag(boma, *FIGHTER_RYU_INSTANCE_WORK_ID_FLAG_IS_HASOGEKI) {
            ArticleModule::generate_article(boma, *FIGHTER_RYU_GENERATE_ARTICLE_HADOKEN, false, -1);
...
pub fn install() {
    ...
    .acmd("game_specialn", ssbexo_ryu_hadoken_acmd, Low)
    .acmd("game_specialairn", ssbexo_ryu_hadoken_acmd, Low)
    .install()
    ;
}