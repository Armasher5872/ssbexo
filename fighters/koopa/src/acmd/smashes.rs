...
unsafe extern "C" fn ssbexo_koopa_forward_smash_expression(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 20.0);
    if is_excute(agent) {
        ControlModule::set_rumble(boma, Hash40::new("rbkind_nohitll"), 0, false, 0);
    }
    frame(lua_state, 22.0);
    if is_excute(agent) {
...
pub fn install() {
    ...
    .acmd("game_attackhi4", ssbexo_koopa_up_smash_acmd, Low)
    .acmd("game_attacklw4", ssbexo_koopa_down_smash_acmd, Low)
    .install()
    ;
}