...
unsafe extern "C" fn koopa_special_lw_a_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if WorkModule::is_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_LW_FLAG_FROM_GR) {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, log_mask_flag, *FIGHTER_STATUS_ATTR_START_TURN as u32, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_LW as u32, 0);
    0.into()
}
...
unsafe extern "C" fn koopa_special_lw_a_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    if WorkModule::is_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_LW_FLAG_FROM_GR) {
        MotionModule::change_motion(boma, Hash40::new("special_air_lw"), 30.0, 1.0, false, 0.0, false, false);
        WorkModule::off_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_LW_FLAG_FROM_GR);
    }
    else {
...
unsafe extern "C" fn koopa_special_lw_a_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if situation_kind != *SITUATION_KIND_AIR {
        if WorkModule::is_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_LW_FLAG1) {
