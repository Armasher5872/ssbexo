...
unsafe extern "C" fn koopa_special_n_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    StatusModule::init_settings(boma, smash::app::SituationKind(*SITUATION_KIND_NONE), *FIGHTER_KINETIC_TYPE_UNIQ, *GROUND_CORRECT_KIND_KEEP as u32, smash::app::GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, 0, 0, 0, 0);
...
unsafe extern "C" fn koopa_special_n_init_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    WorkModule::set_int(boma, -1, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_STEP);
    WorkModule::set_int(boma, -1, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_PREV_STEP);
    WorkModule::set_int(boma, -1, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_PREV_GENERATE_KIND);
    WorkModule::set_int(boma, -1, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_SE1_HANDLE);
    WorkModule::set_int(boma, -1, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_SE2_HANDLE);
    if kind != *FIGHTER_KIND_KIRBY {
        if kind != *FIGHTER_KIND_KOOPAG {
            WorkModule::set_int64(boma, hash40("se_koopa_special_n02") as i64, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_HEAD_NODE);
            WorkModule::set_int64(boma, hash40("head") as i64, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_HEAD_NODE);
        }
        else {
            WorkModule::set_int64(boma, hash40("se_koopag_special_n02") as i64, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_HEAD_NODE);
            WorkModule::set_int64(boma, hash40("head") as i64, *FIGHTER_KOOPA_STATUS_BREATH_WORK_INT_HEAD_NODE);
        }
    }
    else {
...
unsafe extern "C" fn koopa_special_n_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_SPECIAL_N_DISABLE);
...
unsafe extern "C" fn koopa_special_n_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_SPECIAL_N_DISABLE);
    0.into()
}

unsafe extern "C" fn koopa_special_n_exit_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    WorkModule::on_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_SPECIAL_N_DISABLE);
    0.into()
}
