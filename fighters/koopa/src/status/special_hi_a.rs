...
unsafe extern "C" fn koopa_special_hi_a_pre_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(boma, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, log_mask_flag, *FIGHTER_STATUS_ATTR_START_TURN as u32, *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_HI as u32, 0);
    0.into()
}

unsafe extern "C" fn koopa_special_hi_a_main_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    let boma = fighter.module_accessor;
    let special_hi_use_count = WorkModule::get_int(boma, *FIGHTER_KOOPA_INSTANCE_WORK_ID_INT_SPECIAL_HI_USE_COUNT);
    if WorkModule::is_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_HI_FLAG_MOT_RESTART) {
        if special_hi_use_count == 1 {
            MotionModule::change_motion(boma, Hash40::new("special_air_hi_fall"), 0.0, 1.0, false, 0.0, false, false);
        }
        else {
            MotionModule::change_motion(boma, Hash40::new("special_air_hi"), 0.0, 1.0, false, 0.0, false, false);
        }
        WorkModule::off_flag(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_HI_FLAG_MOT_RESTART);
    }
    else {
        if special_hi_use_count == 1 {
            MotionModule::change_motion_inherit_frame(boma, Hash40::new("special_air_hi_fall"), -1.0, 1.0, 0.0, false, false);
        }
        else {
    ...
    WorkModule::set_float(boma, 0.0, *FIGHTER_KOOPA_STATUS_SPECIAL_HI_WORK_FLOAT_JUMP_RESTART_FRAME);
...
unsafe extern "C" fn koopa_special_hi_a_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let boma = fighter.module_accessor;
    let special_hi_use_count = WorkModule::get_int(boma, *FIGHTER_KOOPA_INSTANCE_WORK_ID_INT_SPECIAL_HI_USE_COUNT);
    let jump_restart_frame = WorkModule::get_float(boma, *FIGHTER_KOOPA_STATUS_SPECIAL_HI_WORK_FLOAT_JUMP_RESTART_FRAME);
    let special_hi_jump_restart_start_frame = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_jump_restart_start_frame"));
    let special_hi_jump_restart_end_frame = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_jump_restart_end_frame"));
    let special_hi_jump_speed_y = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_jump_speed_y"));
    let special_hi_jump_restart_prohibition_frame = WorkModule::get_param_float(boma, hash40("param_special_hi"), hash40("special_hi_jump_restart_prohibition_frame"));
    ...
    if situation_kind != *SITUATION_KIND_GROUND {
        if current_frame >= special_hi_jump_restart_start_frame && current_frame <= special_hi_jump_restart_end_frame {
            if ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_SPECIAL) && special_hi_jump_restart_prohibition_frame < current_frame-jump_restart_frame {
                WorkModule::set_float(boma, current_frame, *FIGHTER_KOOPA_STATUS_SPECIAL_HI_WORK_FLOAT_JUMP_RESTART_FRAME);
...
unsafe extern "C" fn koopa_special_hi_a_end_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_hi_use_count = WorkModule::get_int(boma, *FIGHTER_KOOPA_INSTANCE_WORK_ID_INT_SPECIAL_HI_USE_COUNT);
    if special_hi_use_count < 1 {
        WorkModule::inc_int(boma, *FIGHTER_KOOPA_INSTANCE_WORK_ID_INT_SPECIAL_HI_USE_COUNT);
    }
