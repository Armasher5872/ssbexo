...
unsafe extern "C" fn ssbexo_lucario_dair_acmd(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        SET_SPEED_EX(agent, speed_x, speed_y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
...
pub fn install() {
    ...
    .game_acmd("game_attackairb", ssbexo_lucario_bair_acmd, Low)
    .game_acmd("game_attackairlw", ssbexo_lucario_dair_acmd, Low)
    .install()
    ;
}